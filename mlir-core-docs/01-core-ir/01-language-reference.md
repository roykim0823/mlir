...
> **Section:** Core IR · document 1 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/LangRef/](https://mlir.llvm.org/docs/LangRef/) · source [`mlir/docs/LangRef.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/LangRef.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The single most important document in the set, and the one everything else assumes. It defines
operations, regions, blocks, values, types, attributes, the textual syntax and the well-formedness
rules. Deliberately written as a dry reference: it tells you what is legal, not what is wise.

Read it once end to end even if you retain only half, then return to it as a lookup. The sections
on region semantics (SSACFG versus graph regions) and on the distinction between inherent and
discardable attributes are the ones most often skipped and most often needed later.

**Read first**

- [The MLIR Mental Model](../README.md#the-mlir-mental-model)

**What you should be able to do after this page**

- Read any `.mlir` file and account for every token in it.
- State precisely what distinguishes an operand from an attribute, and a region from a block.
- Say whether a given construct is a core-IR guarantee or a dialect-specific convention.
...
[rationale documentation](../08-rationale/02-mlir-rationale.md),
[glossary](https://mlir.llvm.org/getting_started/Glossary/), and other content are hosted
elsewhere.
...
in a given [kind of region](07-interfaces.md#regionkindinterfaces)). Operations
...
using familiar concepts of compiler [Passes](https://mlir.llvm.org/docs/Passes/). Enabling an arbitrary
...
semantics to be described abstractly using [Traits](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits) and
[Interfaces](07-interfaces.md), enabling transformations to operate on operations
...
[SSA dominance](https://github.com/llvm/llvm-project/blob/main/mlir/docs/<https:/en.wikipedia.org/wiki/Dominator_(graph_theory)>).
Another example is the [IsolatedFromAbove trait](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits#isolatedfromabove),
...
[Symbols](04-symbols-and-symbol-tables.md) and have scoping rules dependent on symbol
attributes.
...
For example, the [Affine dialect](https://mlir.llvm.org/docs/Dialects/Affine/) defines the namespace:
`affine`.
...
certain passes. MLIR provides a [framework](../03-passes-and-rewriting/07-dialect-conversion.md) to convert
...
[target-independent operations](https://mlir.llvm.org/docs/Dialects/MemRef/),
[affine operations](https://mlir.llvm.org/docs/Dialects/Affine/), and
...
The [builtin dialect](https://mlir.llvm.org/docs/Dialects/Builtin/) defines a select few operations that
...
\[[more rationale](../08-rationale/02-mlir-rationale.md#block-arguments-vs-phi-nodes)\].
...
[RegionKindInterface](07-interfaces.md#regionkindinterfaces).
...
[OpTrait::IsolatedFromAbove](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits#isolatedfromabove), or a custom verifier.
...
[RegionKind::SSACFG](07-interfaces.md#regionkindinterfaces). Informally, these
...
[RegionKind::Graph](07-interfaces.md#regionkindinterfaces). Graph regions are
...
See [here](../02-defining-dialects/03-defining-attributes-and-types.md) to learn how to define
dialect types.

### Builtin Types

The [builtin dialect](https://mlir.llvm.org/docs/Dialects/Builtin/) defines a set of types that are
...
See [Tokens](02-lexical-tokens.md) for details on ODS integration and examples.
...
exposed through [Interfaces](07-interfaces.md) accessors and other methods.
...
[`cmpi` operation](https://mlir.llvm.org/docs/Dialects/ArithOps/#arithcmpi-arithcmpiop). Each operation
...
See [here](../02-defining-dialects/03-defining-attributes-and-types.md) on how to define dialect
attribute values.

### Builtin Attribute Values

The [builtin dialect](https://mlir.llvm.org/docs/Dialects/Builtin/) defines a set of attribute values
...
The document is organised bottom-up, starting with lexical structure. If that ordering does not
suit you, this route through it is more immediately useful:

1. **High-Level Structure** — the operation/region/block nesting. Everything else hangs off this.
2. **Operations** — the generic form `%res = "dialect.op"(%args) {attrs} : (types) -> types`. Learn
   to read the generic form; every custom syntax you meet is sugar over it, and
   `mlir-opt --mlir-print-op-generic` will show you the underlying form whenever custom syntax
   confuses you.
3. **Regions** — in particular the SSACFG/graph distinction and the value-scoping rules.
4. **Type System** and **Attributes** — then the builtin catalogue below.
5. **Blocks**, **Dominance**, and the lexical sections as needed.

### Points that repay a second reading

**Generic vs custom assembly.** Every operation has a generic form; most also define a pretty form.
Two spellings, one operation. When debugging a parser problem, print generic.

**Value scoping and isolation.** Nested regions may refer to values defined outside them *unless*
the enclosing operation carries `IsolatedFromAbove`. That trait is what makes a function body a
self-contained unit and what allows the pass manager to run passes on functions in parallel. It is
a load-bearing property, not a detail.

**Dominance in graph regions.** Graph regions relax the requirement that definitions dominate uses.
If you are importing a dataflow graph where a cycle is meaningful, this is the escape hatch; if you
are writing ordinary control flow, you want SSACFG and its guarantees.

**Attributes are typed and uniqued.** Two structurally identical attributes are pointer-equal. This
makes comparison cheap and makes attributes usable as hash keys, and it means attributes are
immutable — you construct a new one rather than mutating.

**Type and attribute aliases.** `!foo = tensor<4xf32>` and `#bar = dense<...>` exist purely to keep
large files readable. They have no semantic content and are expanded at parse time.

### The builtin type and attribute catalogue

The dedicated `builtin` dialect page is outside this collection's scope, so here is the working
subset you need. These are always available, in every context, without loading a dialect.

**Types**

| Type | Notes |
|------|-------|
| `i1`, `i8`, `i32`, `i64`, … | signless integers — the *type* has no signedness; the *operation* decides. `si32`/`ui32` exist for frontends that need to carry signedness, but core arithmetic is signless. |
| `f16`, `bf16`, `f32`, `f64`, `f8E4M3FN`, … | floating point, including the low-precision formats used in machine learning |
| `index` | target-defined width, used for loop bounds and subscripts |
| `tensor<4x8xf32>`, `tensor<?x8xf32>`, `tensor<*xf32>` | pure values: no address, no aliasing. `?` is a dynamic dimension; `*` is unranked |
| `memref<4x8xf32, strided<[8,1], offset: 0>, 3>` | storage reference: shape, optional layout, optional memory space |
| `vector<4xf32>`, `vector<4x8xf32>`, `vector<[4]xf32>` | fixed-shape SIMD values; `[4]` marks a scalable dimension |
| `complex<f32>`, `tuple<...>`, `none` | as named |
| `(i32, f32) -> i64` | function type |

**`index` is not an integer type.** Mixing `index` and `i64` requires an explicit cast. This is
deliberate: it keeps target word size out of the high-level IR.

**Signless integers surprise people.** `i32` is neither signed nor unsigned. `arith.divsi` and
`arith.divui` are different operations on the same type. This avoids the type-conversion noise that
signed/unsigned types cause in other IRs.

**`tensor` vs `memref` is the distinction to internalise.** Same shape syntax, entirely different
semantics. A tensor has no identity — the compiler may duplicate or eliminate it freely. A memref
names storage, so it carries aliasing, a layout describing how logical indices map to linear
offsets, and a memory space tag distinguishing (for instance) scratchpad from main memory. Moving
between them is the bufferization phase.

**Shaped types.** `tensor`, `memref` and `vector` all implement `ShapedType`, so shape-generic code
works across them. Unranked types (`tensor<*xf32>`) are far more restrictive than dynamic ones and
are usually an import artefact worth eliminating early.

**Attributes**

| Attribute | Notes |
|-----------|-------|
| `42 : i32`, `3.0 : f32` | `IntegerAttr`, `FloatAttr` |
| `"text"` | `StringAttr` |
| `[1, 2, 3]` | `ArrayAttr` — heterogeneous, each element an attribute |
| `array<i64: 1, 2, 3>` | `DenseArrayAttr` — homogeneous and compact; prefer it for shape-like data |
| `{a = 1, b = "x"}` | `DictionaryAttr` |
| `dense<[[1,2],[3,4]]> : tensor<2x2xi32>` | `DenseElementsAttr` — constant tensor data, with a splat form for uniform values |
| `dense_resource<weights>` | `DenseResourceElementsAttr` — bulk data kept outside the uniquer |
| `@symbol`, `@outer::@inner` | `SymbolRefAttr` |
| `affine_map<(d0, d1) -> (d1, d0)>` | `AffineMapAttr` |
| `#loc`, `unknown` | location attributes |

`DenseResourceElementsAttr` is worth knowing before you first try to debug a module containing real
model weights: it keeps the payload out of the attribute uniquer so printing the module does not
dump megabytes of hex.

**`ModuleOp` is not special.** It is an operation with `IsolatedFromAbove`, a symbol table and a
single region. Any operation with the right traits can serve as a translation unit; the pass
manager is not hard-coded to modules.

### Where it stops

The Language Reference describes the *builtin* structure. It does not describe how to define your
own operations (section 02), what the standard dialects mean (out of scope here — see
<https://mlir.llvm.org/docs/Dialects/>), or how to transform anything (section 03).

A common early mistake is to look for `add` or `for` here. They are not core language constructs;
they live in the `arith` and `scf` dialects respectively, and they are not privileged.


---

[← Vocabulary Quick Reference](../README.md#vocabulary-quick-reference) · [Index](../README.md) · [Lexical Tokens →](02-lexical-tokens.md)
