...
> **Section:** Core IR · document 4 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/SymbolsAndSymbolTables/](https://mlir.llvm.org/docs/SymbolsAndSymbolTables/) · source [`mlir/docs/SymbolsAndSymbolTables.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/SymbolsAndSymbolTables.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

SSA values cover data flow within a region, but they cannot express "call the function named `@foo`
defined elsewhere in the module". Symbols are MLIR's answer: named entities registered in a symbol
table, referenced by `SymbolRefAttr` rather than by `Value`.

This matters as soon as you have functions, global variables, or any cross-module reference. It
also governs how you rename things safely — `SymbolTable::replaceAllSymbolUses` exists because
naively string-replacing a symbol name will silently miss nested references.
...
- Define an operation that is a symbol, and one that holds a symbol table.
- Rename or delete a symbol without leaving dangling references.
- Understand why symbol visibility affects what optimizations are legal.

---

## Upstream documentation

With [Regions](01-language-reference.md#regions), the multi-level aspect of MLIR is
...
[pass manager](../03-passes-and-rewriting/01-pass-infrastructure.md#pass-manager). One advantage of the MLIR
...
[`func.func`](https://mlir.llvm.org/docs/Dialects/Builtin/#func-mlirfuncop). `func.func` defines a
symbol name, which is [referred to](#referencing-a-symbol) by operations like
[`func.call`](https://mlir.llvm.org/docs/Dialects/Func/#funccall-callop).
...
[`SymbolRefAttr`](https://mlir.llvm.org/docs/Dialects/Builtin/#symbolrefattr) attribute. A symbol
...
*   References may appear in more places than the operand list; including
    [nested attribute dictionaries](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Dialects/Builtin.md/dictionaryattr),
    [array attributes](https://mlir.llvm.org/docs/Dialects/Builtin/#arrayattr), etc.
...
[`llvm.mlir.addressof`](https://mlir.llvm.org/docs/Dialects/LLVM/#llvmmliraddressof-mlirllvmaddressofop)
is one example of such an operation.

See the `LangRef` definition of the
[`SymbolRefAttr`](https://mlir.llvm.org/docs/Dialects/Builtin/#symbolrefattr) for more information about
...
A `Value` is a def-use edge in a graph: precise, verified, and local to a region tree. It is the
wrong tool for a reference that must survive being written to disk, that may be forward-declared,
or that names something in a different translation unit. Symbols trade the def-use guarantee for
those properties, and pay for it by requiring explicit bookkeeping when names change.

### The three pieces

| Piece | Trait / type | Meaning |
|-------|--------------|---------|
| Symbol | `Symbol` op interface, `sym_name` attribute | an operation that has a name |
| Symbol table | `SymbolTable` trait | an operation whose region holds symbols, names unique within it |
| Reference | `SymbolRefAttr` (`@foo`, `@outer::@inner`) | an attribute naming a symbol |

Symbol tables nest, which is what the `@outer::@inner` syntax expresses. A module inside a module
gives you a two-level namespace.

### Visibility, and why it is not cosmetic

- `public` (default) — may be referenced from outside; you cannot prove all uses are visible, so
  you cannot delete it as dead and cannot change its signature.
- `private` — all uses are within this symbol table. Safe to inline, specialise, rename or delete.
- `nested` — visible to enclosing tables but not beyond.

Marking internal functions `private` is often the change that suddenly enables inlining and
dead-function elimination in a pipeline that appeared to do nothing.

### Practical rules

**Never rename by string substitution.** Use `SymbolTable::replaceAllSymbolUses`, which walks
nested references correctly. Hand-rolled renaming that misses a nested `@a::@b` produces IR that
verifies locally and fails at translation time.

**Symbol uses are not indexed by default.** `SymbolTable::getSymbolUses` scans; on a large module
inside a loop this is quadratic. Build a `SymbolUserMap` once if you need repeated queries.

**`SymbolTable` construction is not free.** Constructing one builds a name-to-op map. Passes that
construct a fresh `SymbolTable` per operation show up in profiles; hoist it, or use the pass
manager's cached analysis.

**Symbol DCE needs correct visibility to work.** If `symbol-dce` is not removing anything, check
that your unused symbols are actually marked private — that is almost always the cause.


---

[← Understanding the IR Structure](03-understanding-the-ir-structure.md) · [Index](../README.md) · [Traits →](05-traits.md)
