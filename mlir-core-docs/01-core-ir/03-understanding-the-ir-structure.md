...
> **Section:** Core IR · document 3 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Tutorials/UnderstandingTheIRStructure/](https://mlir.llvm.org/docs/Tutorials/UnderstandingTheIRStructure/) · source [`mlir/docs/Tutorials/UnderstandingTheIRStructure.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Tutorials/UnderstandingTheIRStructure.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Filed upstream as a tutorial; in practice it is the C++-side companion to the Language Reference,
which is why it sits here. It shows how to traverse a module: iterate regions, blocks and
operations, inspect operands, results, attributes and uses, and walk the tree with `walk()`.

This is the first page where you write code that touches MLIR, and the fastest route from "I can
read the textual form" to "I can manipulate it".
...
- Traverse a module in C++ and print structural information about it.
- Use `walk()` with the right callback signature and interrupt it correctly.
- Move between a generic `Operation*` and a concrete op class safely.
...
[High Level Structure](01-language-reference.md#high-level-structure), this document
illustrates this structure through examples, and introduces at the same time the
C++ APIs involved in manipulating it.

We will implement a [pass](../03-passes-and-rewriting/01-pass-infrastructure.md#operation-pass) that traverses any
...
As defined in the
[language reference](01-language-reference.md#high-level-structure),
...
### The API shapes worth memorising

```cpp
op->getName()                     // OperationName
op->getNumOperands(), getOperand(i), getOperands()
op->getNumResults(),  getResult(i),  getResults()
op->getAttrs(), op->getAttr("name"), op->getAttrOfType<IntegerAttr>("name")
op->getRegions()                  // then block iteration, then ops again
op->getBlock(), op->getParentOp(), op->getParentOfType<func::FuncOp>()
value.getDefiningOp()             // null if it is a BlockArgument
value.getUsers(), value.getUses() // use-def chains, both directions
```

**Casting.** `dyn_cast<scf::ForOp>(op)` returns a null op class on failure; `isa<>` tests; `cast<>`
asserts. Op classes are lightweight handles wrapping `Operation*`, so pass them by value and do not
worry about copies. Note that current LLVM requires the free functions (`dyn_cast<T>(op)`) — the
member form (`op.dyn_cast<T>()`) has been removed, which is a common source of build breakage when
following older examples.

**`walk()` traverses nested regions.** Post-order by default; ask for `WalkOrder::PreOrder`
explicitly when order matters. Returning `WalkResult::interrupt()` stops the traversal and makes the
whole `walk` return `interrupted()`. A typed callback filters for you:

```cpp
module.walk([](func::CallOp op) { /* only calls */ });
```

**Mutating during a walk is where bugs live.** Erasing the operation you are currently visiting
invalidates the iterator. Collect into a `SmallVector` first and mutate afterwards, or use the
rewriter infrastructure ([Pattern Rewriting](../03-passes-and-rewriting/03-pattern-rewriting.md))
which is built to handle this. This is probably the most common crash for people new to the C++ API.

**Use-def chains are cheap and intrusive.** `value.getUsers()` is a linked-list walk with no
allocation, so use-based queries are inexpensive. `value.hasOneUse()` and `use_empty()` are the
guards you will reach for constantly when deciding whether a rewrite is profitable.

### Builders

The other half of manipulation is construction:
...
Insertion points are the thing to be careful with. `setInsertionPointAfter(op)`,
`setInsertionPointToEnd(block)` and the RAII `OpBuilder::InsertionGuard` — which restores the
previous insertion point on scope exit — are what keep construction code from silently emitting
operations in the wrong place. Use the guard whenever you temporarily move the insertion point.

Inside a pattern, use the `PatternRewriter` rather than a bare `OpBuilder`, for the reasons
explained in [Pattern Rewriting](../03-passes-and-rewriting/03-pattern-rewriting.md).


---

[← Lexical Tokens](02-lexical-tokens.md) · [Index](../README.md) · [Symbols and Symbol Tables →](04-symbols-and-symbol-tables.md)
