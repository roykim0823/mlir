...
> **Section:** Core IR · document 2 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Tokens/](https://mlir.llvm.org/docs/Tokens/) · source [`mlir/docs/Tokens.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Tokens.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

A short companion to the Language Reference describing the lexical layer: identifiers, keywords,
integer and float literals, string literals, and punctuation.

You can skip this on a first pass. It becomes relevant the moment you write a custom parser or a
declarative assembly format, because the format directives are defined in terms of these token
kinds and parser error messages name them.
...
- Interpret parser error messages that name a token kind.
- Choose valid identifiers and literal spellings when designing custom syntax.
...
[LangRef section on tokens](01-language-reference.md#token-type).
...
The [LangRef](01-language-reference.md#token-type) requires that a token never appears as a
...
Three practical consequences of the token rules.

**Bare identifiers vs quoted names.** A symbol name that does not fit the bare-identifier grammar
must be quoted. This matters when importing from a language with a permissive identifier alphabet —
mangled C++ names, dotted attribute paths — and it is a common source of round-trip failures in
importers that emit names without escaping them.

**Suffixed literals are not single tokens.** `42 : i32` is an integer literal plus a type
annotation. The parser is largely type-directed, which is why an omitted type annotation usually
surfaces as a confusing "expected type" error rather than a message about the literal.

**Sigils encode the namespace.** `%` for values, `^` for blocks, `@` for symbols, `#` for
attributes, `!` for types. Once internalised, this makes unfamiliar IR far quicker to skim —
you can tell what kind of thing a name refers to without reading its definition.

When designing custom assembly, stay inside these token kinds. Syntax requiring new token types
cannot be expressed in the declarative `assemblyFormat` and forces you into hand-written
`parse`/`print` methods, which then need hand-maintained round-trip tests.


---

[← MLIR Language Reference](01-language-reference.md) · [Index](../README.md) · [Understanding the IR Structure →](03-understanding-the-ir-structure.md)
