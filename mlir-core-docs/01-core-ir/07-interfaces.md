...
> **Section:** Core IR · document 7 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Interfaces/](https://mlir.llvm.org/docs/Interfaces/) · source [`mlir/docs/Interfaces.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Interfaces.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The most important document in this section after the Language Reference, and arguably the key to
MLIR's whole design. Interfaces let a pass operate on operations it has never heard of, by asking
them questions through a virtual API instead of switching on their names.

If your dialect implements `MemoryEffectsOpInterface`, existing DCE and CSE work on it. If it
implements `LoopLikeOpInterface`, loop-invariant code motion works on it. If it implements
`DestinationStyleOpInterface`, bufferization understands it. None of that infrastructure contains a
line of code mentioning your dialect.

This is where to spend real time before designing anything of your own. Failing to use interfaces
is the most common way to end up reimplementing infrastructure that already exists.

**Read first**

- [Traits](05-traits.md)
- [Understanding the IR Structure](03-understanding-the-ir-structure.md)

**What you should be able to do after this page**

- Choose between op, type, attribute and dialect interfaces.
- Implement an existing interface on your operation, in ODS and in C++.
- Define a new interface, including default method implementations.
- Use external models to implement an interface for an op you do not own.
...
[`Operation Definition Specification`](../02-defining-dialects/02-operation-definition-specification.md) documentation.
...
[`Trait`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits) class that can be appended to the trait list of that
object.
...
*   Verifier (`verify`)
    ...
    -   The structure of this code block corresponds 1-1 with the structure of a
        [`Trait::verifyTrait`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits) method.
...
*   DefaultImplementation (Optional)
    ...
    -   This implementation is placed within the `Trait` class that is attached
        ...
        [`Trait`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits) method.
    -   `ConcreteAttr`/`ConcreteOp`/`ConcreteType` is an implicitly defined
        `typename` that can be used to refer to the type of the derived IR
        entity currently being operated on.
...
def MyInterface : OpInterface<"MyInterface"> {
  ...
  let methods = [
    ...
    InterfaceMethod<[{
      ...
      As detailed in [Traits](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits), given that each operation implementing
...
*   `RegionKindInterface` - Used to describe the abstract semantics of regions.
    -   `RegionKind getRegionKind(unsigned index)` - Return the kind of the
        ...
        -   RegionKind::SSACFG - represents an
            [SSA-style control flow](01-language-reference.md#control-flow-and-ssacfg-regions) region
...
*   `SymbolOpInterface` - Used to represent
    [`Symbol`](04-symbols-and-symbol-tables.md#symbol) operations which reside
...
*   `SymbolUserOpInterface` - Used to represent operations that reference
    [`Symbol`](04-symbols-and-symbol-tables.md#symbol) operations. This provides the
...
| Kind | Attached to | Typical use |
|------|-------------|-------------|
| `OpInterface` | operations | "how do I tile you", "what memory do you touch" |
| `TypeInterface` | types | "what is your element type", "are you shaped" |
| `AttrInterface` | attributes | "what is your value as an integer", alias behaviour |
| `DialectInterface` | dialects | inlining policy, LLVM translation, folding hooks |

`DialectInterface` is the one people forget exists. `DialectInlinerInterface` is how a dialect tells
the generic inliner whether inlining a call into a region is legal and how to handle the
terminator — without the inliner knowing anything about the dialect.

### External models: the escape hatch that matters

You can implement an interface for an operation you do not own, from outside its dialect, via
`ExternalModel`. This is how the bufferization framework teaches upstream dialects to bufferize
without those dialects depending on it, and it is how you avoid the dependency inversion that would
otherwise force upstream dialects to know about your downstream analysis.

The cost is registration: the model must be attached before use, usually in dialect registration or
in your tool's `main`. Forgetting registration produces a runtime failure ("operation does not
implement interface") that is not obvious from the IR — the most common failure mode with external
models. Wire the registration in at the same time you write the model.

### Interfaces most worth knowing by name

| Interface | Enables |
|-----------|---------|
| `MemoryEffectsOpInterface` | DCE, CSE, hoisting, alias analysis |
| `LoopLikeOpInterface` | loop-invariant code motion, loop utilities |
| `RegionBranchOpInterface` | control-flow analysis across regions, dataflow |
| `BranchOpInterface` | CFG analyses, successor operand mapping |
| `CallOpInterface` / `CallableOpInterface` | inlining, call-graph construction |
| `FunctionOpInterface` | generic signature manipulation, used by dialect conversion |
| `InferTypeOpInterface` | result-type inference in builders and parsers |
| `DestinationStyleOpInterface` | bufferization and in-place update reasoning |
| `TilingInterface` | generic tiling and fusion |
| `PromotableAllocationOpInterface` | mem2reg / SROA |
| `ViewLikeOpInterface` | aliasing through views |
| `SymbolOpInterface` | symbol infrastructure |
| `OpAsmOpInterface` / `OpAsmDialectInterface` | result name hints, type and attribute aliases |
| `BytecodeDialectInterface` | versioned binary serialization |

### Design guidance

**Prefer implementing an existing interface over inventing one.** The upstream set is large and the
value is entirely in the network effect. A new interface with one implementation and one caller is
just a virtual call with extra ceremony.

**Give default implementations where you can.** ODS lets an interface method carry a default body,
so implementers only override what differs. This is what keeps interfaces with a dozen methods from
being prohibitive to adopt.

**Interfaces have a cost.** Dispatch is indirect and the concept-model machinery is not free. On hot
paths — a fold called on every operation in a fixpoint loop — measure. In practice this is almost
never the bottleneck, but "almost never" is not "never".

**Verification belongs with the interface.** An interface can declare a verifier that runs on every
implementing op, which is how you keep implementations honest about the contract you wrote in prose.

### The practical consequence

When someone reports that a generic pass "does not work on my dialect", the answer is almost always
a missing interface rather than a limitation of the pass. Before writing a custom version of an
existing transformation, check which interface it queries and whether your operations implement it.


---

[← The `Broadcastable` Trait](06-trait-broadcastable.md) · [Index](../README.md) · [Defining Dialects →](../02-defining-dialects/01-defining-dialects.md)
