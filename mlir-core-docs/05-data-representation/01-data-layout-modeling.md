...
> **Section:** Data Representation · document 1 of 3  
> **Upstream:** [https://mlir.llvm.org/docs/DataLayout/](https://mlir.llvm.org/docs/DataLayout/) · source [`mlir/docs/DataLayout.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DataLayout.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Sooner or later a transformation needs to know how many bytes a type occupies, what alignment it
requires, or how wide a pointer in a given address space is. Hard-coding those answers ties your pass
to a target; asking the LLVM data layout string directly ties you to the LLVM dialect.

The data layout infrastructure gives a dialect-neutral query interface — `DataLayout` plus
`DataLayoutTypeInterface` and `DataLayoutOpInterface` — with the `dlti` dialect providing the
attributes that carry the actual specification and a scoping rule that lets nested operations
override enclosing specifications.
...
- Query size, alignment and index bit width for a type without hard-coding a target.
- Attach a data layout specification to a module or a nested scope.
- Make a custom type answer layout queries.
...
Data layout specification is an [attribute](../01-core-ir/01-language-reference.md#attributes) that is
...
The [DLTI](https://github.com/llvm/llvm-project/blob/main/mlir/Dialects/DLTIDialect) dialect provides the attributes implementing
...
### Why this is not simply "ask LLVM"

Three reasons. First, layout questions arise well above the LLVM dialect — a bufferization or
vectorization decision may need element sizes while the IR is still in tensor and buffer form.
Second, heterogeneous compilation means one module can contain code for a host and for a device with
different pointer widths and different address-space semantics, so a single global layout string is
not expressive enough. Third, custom types from your own dialect have no LLVM counterpart until
lowering, yet still have a size.

### The scoping rule

Specifications attach to operations carrying `DataLayoutOpInterface` — a module, typically — and
nested scopes may override. A query resolves by walking outward to the nearest enclosing
specification. This is what lets a device module inside a host module declare a different pointer
size, and it is the mechanism you need for anything targeting an accelerator with its own address
spaces.

### Practical notes

**Construct `DataLayout` once.** It caches; constructing it per query inside a walk is a recognisable
performance mistake.

**A missing specification is not an error.** Queries fall back to defaults, so an absent layout
silently yields plausible-but-wrong answers rather than failing loudly. If your pipeline depends on
layout, verify the specification is present rather than trusting the query result.

**Alignment comes in two flavours.** ABI alignment (required) and preferred alignment (desirable);
they differ for some types on some targets, and picking the wrong one produces either miscompiles or
needless padding.

**Sub-byte types are where the bit/byte distinction bites.** `getTypeSizeInBits` is authoritative;
the byte size rounds up. For low-precision inference work with `i4` or narrower element types, read
that part carefully — an `i4` element is 4 bits, and whether a buffer of them is packed two per byte
or padded to one per byte is a decision with large consequences that the layout query alone does not
settle.

**Custom types implement `DataLayoutTypeInterface`,** providing `getTypeSize`, `getTypeSizeInBits`,
`getABIAlignment` and `getPreferredAlignment`.

### Relationship to the `dlti` dialect

`dlti` supplies the attributes: a data layout specification is a `#dlti.dl_spec` attached to the
module, containing entries mapping types and identifiers to values. The query interface reads it. You
will also see `dlti` used to carry general target information — device properties, capabilities —
beyond strict layout, which is a reasonable use of the same scoping mechanism.


---

[← LLVM IR Target](../04-memory-and-lowering/03-llvm-ir-target.md) · [Index](../README.md) · [Quantization →](02-quantization.md)
