...
> **Section:** Data Representation · document 3 of 3  
> **Upstream:** [https://mlir.llvm.org/docs/BytecodeFormat/](https://mlir.llvm.org/docs/BytecodeFormat/) · source [`mlir/docs/BytecodeFormat.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/BytecodeFormat.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

MLIR's textual form is the one you read; the bytecode form is the one you should ship. It is
substantially faster to parse, much smaller, and — critically — versioned, so a producer and a
consumer built from different revisions can interoperate within the compatibility window a dialect
declares.

Read this if you serialize IR anywhere in your system: an ahead-of-time compiler writing an
intermediate artefact, a caching layer, a compiler split across process boundaries, or a model format
that embeds MLIR.
...
- Explain the section layout of a bytecode file and what each section holds.
- Implement versioned encoding for a dialect's custom attributes and types.
- Reason about forward and backward compatibility when a dialect changes.
...
A bytecode file is a header plus a set of sections: string table, dialect table, attribute and type
tables, IR section, and optional resource sections. Strings, attributes and types are interned into
tables and referenced by index, which is where most of the size reduction comes from — a module that
repeats a long tensor type a thousand times stores it once.

Resources are the mechanism for bulk data that should not be interned, such as model weights. They
live in their own section and can be read lazily, so tooling can inspect a module's structure without
paging in hundreds of megabytes of parameters.

### Versioning, and the part people get wrong

A dialect participates via `BytecodeDialectInterface` and may declare a version. On read, the dialect
can upgrade older encodings. This is the mechanism that makes bytecode a supportable artefact format
rather than a build-specific cache.

Two consequences that are easy to miss:

- **Compatibility is a per-dialect promise, not a framework guarantee.** The framework gives you the
  machinery; whether your dialect can read last year's files depends entirely on whether you wrote
  the upgrade path when you made the change. If you ship bytecode to users, that discipline is now
  part of your release process.
- **Round-tripping through text is not equivalent.** Text loses nothing semantically, but resource
  handling and some encodings differ, so "it round-trips through `mlir-opt`" does not prove your
  bytecode reader is correct. Test bytecode round-trips explicitly.

### When to use it

| Situation | Format |
|-----------|--------|
| Tests, debugging, anything a human reads | text |
| Build cache, cross-process IPC, shipped artefacts | bytecode |
| Long-term archival across compiler versions | bytecode, with a tested upgrade path per dialect |

`mlir-opt --emit-bytecode` writes it; the parser detects the format automatically on read, so tools
generally need no changes to accept it.

### Lazy loading

The format is designed so that a reader can skip regions it does not need. For tooling that only
wants to inspect a module's function signatures, or only wants to load one function out of a large
module, this is a substantial win — and it is a reason to prefer bytecode for any artefact that will
be queried more often than it is fully compiled.


---

[← Quantization](02-quantization.md) · [Index](../README.md) · [Using `mlir-opt` →](../06-tooling-and-debugging/01-using-mlir-opt.md)
