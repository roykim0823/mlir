...
> **Section:** Defining Dialects · document 2 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/DefiningDialects/Operations/](https://mlir.llvm.org/docs/DefiningDialects/Operations/) · source [`mlir/docs/DefiningDialects/Operations.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DefiningDialects/Operations.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The longest and most consequential document in this section. ODS is how operations are declared:
arguments, results, regions, successors, traits, verification, builders, folding hooks, assembly
format and documentation, all from one TableGen record, with `mlir-tblgen` generating the C++.

Nearly everything you do when building a dialect goes through this page. Treat it as a reference to
work against with an editor open, not as prose to read through once.
...
- Declare an operation with typed operands, results, attributes and regions.
- Add verification declaratively and know when to fall back to a C++ verifier.
- Write custom builders and understand which ones are generated for you.
- Implement folding, and know how it differs from a canonicalization pattern.
...
[Quickstart tutorial to adding MLIR graph rewrite](../03-passes-and-rewriting/06-quickstart-adding-a-rewrite.md)
for the latter.
...
Both operation traits, [interfaces](../01-core-ir/07-interfaces.md#utilizing-the-ods-framework),
...
*   `functional-type ( inputs , outputs )`
    ...
    -   Formats the `inputs` and `outputs` arguments as a
        [function type](https://mlir.llvm.org/docs/Dialects/Builtin/#functiontype).
...
In MLIR, the [`unit` Attribute](https://mlir.llvm.org/docs/Dialects/Builtin/#unitattr) is special in that it
...
*   The op's operand/return type constraints are modelled along with the op in
    the registry (see [Shape inference](07-shape-inference.md) discussion below),
...
### The anatomy of an op definition

```tablegen
def MyDialect_ConvOp : MyDialect_Op<"conv", [
      Pure,
      DeclareOpInterfaceMethods<InferTypeOpInterface>,
      AttrSizedOperandSegments]> {
  let summary = "one-line description";
  let description = [{ markdown, becomes the generated docs and LSP hover text }];
...
Every line there is a decision the page explains. The ones worth calling out:

**`arguments` mixes operands and attributes.** They are both "inputs" from ODS's perspective, and
the generated accessors do not visually distinguish them — but operands are runtime values and
attributes are compile-time constants. Confusing the two is the most common modelling error in a
first dialect. If a value could ever be computed at run time, it is an operand.

**Variadic and optional need segment attributes.** More than one variadic or optional group requires
`AttrSizedOperandSegments` (or `SameVariadicOperandSize`), because otherwise the flat operand list
cannot be partitioned. The verifier error when you forget is opaque.

**Properties vs attributes.** Recent MLIR stores inherent attributes as *properties*: inline in the
operation rather than in the uniqued dictionary. Faster, and they can hold non-attribute C++ types.
ODS handles the mechanics; the visible consequence is that inherent attributes no longer appear in
generic attribute enumeration unless you ask for the full dictionary — which breaks code that
iterated attributes generically. If you are updating a dialect written a few releases ago, this is a
likely source of surprises.

### Verification: four places, in order of preference

1. **Type constraints in `arguments`/`results`** — `AnyRankedTensor`, `I32`, `Confined<...>`.
   Declarative, generates good messages, costs nothing to maintain.
2. **Traits** — `SameOperandsAndResultType` and friends. Also free.
3. **`let hasVerifier = 1`** — a C++ `verify()` for logic the declarative forms cannot express.
4. **`let hasRegionVerifier = 1`** — runs after region verification, for checks that need the region
   contents to be valid first. Using `verify()` when you need `verifyRegions()` produces crashes on
   malformed input rather than diagnostics.

Push checks as far up that list as they will go.

### Builders

`mlir-tblgen` generates several builders automatically: one taking all result types and operands,
one inferring result types if the op implements `InferTypeOpInterface`, and collective-parameter
variants. Declare extra ones with `let builders = [...]` when the natural call site has different
information — for instance a builder taking an `int64_t` where the op stores an `IntegerAttr`, so
callers do not construct attributes by hand.

A builder that takes exactly the arguments your patterns have to hand is a small piece of API design
that pays off across every pattern you write afterwards.

### Folding vs canonicalization

| | `fold()` | canonicalization pattern |
|---|---|---|
| May create ops? | no (may create constants via the materializer) | yes |
| Operates on | one op, in place | a subgraph |
| Cost | very cheap, runs constantly | more expensive |
| Use for | `x + 0 → x`, constant evaluation | restructuring, multi-op rewrites |

`fold()` returning a `Value` means "replace with this existing value"; returning an `Attribute`
means "replace with a constant of this value", which requires the dialect's constant materializer to
exist. Anything that needs to build new operations belongs in a canonicalization pattern
([Canonicalization](../03-passes-and-rewriting/02-operation-canonicalization.md)).

### Assembly format, briefly

`assemblyFormat` covers most operations and is strongly preferred over hand-written parsers: it
generates both directions, so round-tripping cannot break. The common blockers are optional groups
whose presence is not inferable, and types that must be inferred from an attribute.
[Customizing Assembly Behavior](05-customizing-assembly-behavior.md) covers the escape hatches.
Before writing a custom parser, check whether `custom<Foo>($x)` directives — which let you
hand-write just the awkward fragment — will do.

### Generated code you should look at once

Run `mlir-tblgen --gen-op-decls` and `--gen-op-defs` on your `.td` and read the output for one
operation. Ten minutes doing this removes most of the mystery about where accessors, builders and
verifiers come from, and makes ODS error messages legible — they are emitted by TableGen about the
code it is trying to generate, so knowing the shape of that code is most of the interpretation.


---

[← Defining Dialects](01-defining-dialects.md) · [Index](../README.md) · [Defining Dialect Attributes and Types →](03-defining-attributes-and-types.md)
