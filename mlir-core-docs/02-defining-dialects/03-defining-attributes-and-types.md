...
> **Section:** Defining Dialects · document 3 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/DefiningDialects/AttributesAndTypes/](https://mlir.llvm.org/docs/DefiningDialects/AttributesAndTypes/) · source [`mlir/docs/DefiningDialects/AttributesAndTypes.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DefiningDialects/AttributesAndTypes.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Operations are only half of a dialect. If your abstraction has data not adequately described by the
builtin types — a quantized element type carrying scale and zero point, a token type for
asynchronous dependencies, a layout attribute describing how a tensor maps onto an accelerator's
memory — you define your own types and attributes.

This page covers the ODS records, the parameter system, storage and uniquing, custom parsers and
printers, and the interfaces that let your types participate in generic infrastructure.
...
- Define a parameterized type or attribute and give it readable syntax.
- Explain uniquing and what it means for mutability and equality.
- Choose between a type parameter and an attribute on the operation.
...
[attributes](../01-core-ir/01-language-reference.md#attributes) and [types](../01-core-ir/01-language-reference.md#type-system).


## LangRef Refresher

Before diving into how to define these constructs, below is a quick refresher
from the [MLIR LangRef](../01-core-ir/01-language-reference.md).

### Attributes

Attributes are the mechanism for specifying constant data on operations in
places where a variable is never allowed - e.g. the comparison predicate of a
[`arith.cmpi` operation](https://mlir.llvm.org/docs/Dialects/ArithOps/#arithcmpi-arithcmpiop), or
the underlying value of a [`arith.constant` operation](https://mlir.llvm.org/docs/Dialects/ArithOps/#arithconstant-arithconstantop).
...
the following [Arithmetic AddI operation](https://mlir.llvm.org/docs/Dialects/ArithOps/#arithaddi-arithaddiop):
...
which is an instance of the [Builtin IntegerType](https://mlir.llvm.org/docs/Dialects/Builtin/#integertype).
...
recommendation extends to all of the MLIR constructs, including [Interfaces](../01-core-ir/07-interfaces.md),
Operations, etc.
...
above. See the main [`Trait`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits) documentation for more information
...
`AttrDef` or `TypeDef`. See the main [`Interface`](../01-core-ir/07-interfaces.md)
...
### Uniquing is the thing to internalise

Types and attributes are interned in the `MLIRContext`: two structurally equal instances are the
same pointer. This buys O(1) equality and cheap hashing, and it imposes immutability — there is no
setter, you construct a new instance. It also means construction requires a context, which is why so
many APIs thread `MLIRContext*` around.

Mutable types exist, for genuinely recursive types such as a self-referential struct, and are
covered here — but they are a specialised tool with real sharp edges around thread safety. If you
think you need one, first check whether a symbol reference
([Symbols and Symbol Tables](../01-core-ir/04-symbols-and-symbol-tables.md)) models the recursion
better.

### Choosing where information lives

A recurring design question: does this belong in the *type*, or in an *attribute on the operation*?

| Put it in the type when | Put it in an attribute when |
|---|---|
| It is a property of the value, and every use must agree | It is a property of this particular operation |
| You want the verifier to enforce agreement at every def-use edge for free | Different ops may legitimately disagree |
| It should survive being passed through a function boundary | It is local scheduling or lowering metadata |

Quantization parameters are the canonical example of the left column: an element type carrying scale
and zero point means every operation consuming that value sees the same scale, checked
automatically. Encoding the same information as an attribute on each operation means every pass must
propagate it correctly, and one that forgets produces silently wrong numerics with no diagnostic.

The mirror-image mistake is putting scheduling hints in types, which makes every rewrite that
changes a hint also change types, forcing casts everywhere.

### Parameters and syntax

Parameters are typed fields (`"int64_t":$width`, `ArrayRefParameter<"int64_t">:$shape`). Array
parameters need an allocator so the storage outlives the builder — `ArrayRefParameter` handles the
common case; hand-rolled parameters holding pointers need explicit allocation in the storage
constructor, or you get use-after-free that only shows up under memory pressure.

`assemblyFormat` works for types and attributes too, and is preferred for the same reason as with
operations: one declaration, both directions, no round-trip drift. `struct(...)` handles
keyword-value parameter lists; `custom<...>` handles the awkward fragment without giving up the
generated parser entirely.

Parameters can carry their own verification via `let genVerifyDecl = 1`, which is where you enforce
invariants like "the shape and the strides must have the same length". Doing it here rather than in
every operation that uses the type is the whole point of putting the data in a type.

### Interfaces on types

Do not overlook these. `ShapedType` makes your type work with shape-generic code.
`DataLayoutTypeInterface` ([Data Layout](../05-data-representation/01-data-layout-modeling.md)) lets
it answer size and alignment queries. `MemRefElementTypeInterface` marks it as valid inside a
memref. Same network-effect argument as op interfaces: implement the existing ones and existing
infrastructure starts working.

### Default values and versioning

Adding a parameter to a type in a later release breaks every serialized artefact and every textual
test that spelled the type out. If your type is likely to grow, consider carrying an attribute
dictionary parameter from the start, or plan a bytecode upgrade path
([Bytecode Format](../05-data-representation/03-bytecode-format.md)) before you ship.


---

[← Operation Definition Specification (ODS)](02-operation-definition-specification.md) · [Index](../README.md) · [Constraints →](04-constraints.md)
