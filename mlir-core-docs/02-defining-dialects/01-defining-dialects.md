...
> **Section:** Defining Dialects · document 1 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/DefiningDialects/](https://mlir.llvm.org/docs/DefiningDialects/) · source [`mlir/docs/DefiningDialects/_index.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DefiningDialects/_index.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The entry point for extending MLIR. A dialect is the namespace that owns your operations, types and
attributes, plus a place to hang dialect-level behaviour: materializing constants, parsing and
printing your custom types, controlling the inliner, declaring dependencies on other dialects.

Read it before the Operations page, even though operations are what you actually want to write,
because several of the choices made here — `useDefaultTypePrinterParser`,
`hasConstantMaterializer`, `dependentDialects` — change what the later pages ask of you.
...
- Declare a dialect in TableGen and generate its C++ scaffolding.
- Decide which dialect-level hooks you need and why.
- Register your dialect so tools and passes can see it.

---

## Upstream documentation

This document describes how to define [Dialects](../01-core-ir/01-language-reference.md#dialects).
...
ecosystem. They allow for defining new [attributes](../01-core-ir/01-language-reference.md#attributes),
[operations](../01-core-ir/01-language-reference.md#operations), and [types](../01-core-ir/01-language-reference.md#type-system).
Dialects are used to model a variety of different abstractions; from traditional
[arithmetic](https://mlir.llvm.org/docs/Dialects/ArithOps/) to
[pattern rewrites](https://mlir.llvm.org/docs/Dialects/PDLOps/); and is one of the most fundamental
aspects of MLIR.
...
extends to all of the MLIR constructs, including [Interfaces](../01-core-ir/07-interfaces.md) for example.
...
[documentation for canonicalization](../03-passes-and-rewriting/02-operation-canonicalization.md) for a more in-depth
...
As described by the [MLIR Language Reference](../01-core-ir/01-language-reference.md#attributes),
...
[interface documentation](../01-core-ir/07-interfaces.md#dialect-fallback-for-opinterface).
...
Generally, [canonicalization](../03-passes-and-rewriting/02-operation-canonicalization.md) patterns are specific to individual 
...
See the documentation for [Canonicalization in MLIR](../03-passes-and-rewriting/02-operation-canonicalization.md) for
...
auto verifier = [](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DefiningDialects/function_ref<InFlightDiagnostic()> emitError,
...
### The decisions this page asks you to make

**`dependentDialects`.** Lists dialects that must be loaded whenever yours is, because your
canonicalization patterns or folders create their operations. Getting this wrong produces the
runtime error "dialect not registered" from inside a pattern, which is confusing because the
pattern looked fine in isolation. If your folder ever builds an `arith.constant`, `arith` is a
dependent dialect.

**`hasConstantMaterializer`.** Required if you want folding to turn a computed attribute back into
a constant operation in your dialect. Without it, folds that produce an attribute have nowhere to
put it and are silently less effective — a subtle degradation rather than a failure.

**`useDefaultTypePrinterParser` / `useDefaultAttributePrinterParser`.** Turn these on and the
generated code dispatches to per-type and per-attribute `parse`/`print` methods, giving the familiar
`!my.type<...>` mnemonic dispatch. Off, and you write one dialect-level parser by hand. On is
almost always right.

**`extraClassDeclaration`.** The hatch for adding C++ to the generated class. Convenient, and it
accumulates: dialects that grow a large `extraClassDeclaration` are usually missing an abstraction,
often an interface.

### Registration, and the two failure modes

Registration happens in a `DialectRegistry`, which tools populate before parsing. Two distinct
things can go wrong and their symptoms look similar:

1. **Dialect not registered** — parsing `my.op` fails, or a pattern that creates one crashes. Fix:
   add it to the registry in your tool's `main`, or to `dependentDialects` of the dialect whose
   patterns create it.
2. **Dialect registered but not loaded** — registration makes a dialect *available*; loading
   instantiates it in a context. The parser loads on demand, but code building operations
   programmatically may need `context->loadDialect<MyDialect>()` first.

Interface external models ([Interfaces](../01-core-ir/07-interfaces.md)) are also attached here,
typically in a `registerMyDialectExtensions(registry)` free function — keep that next to the dialect
registration so the two cannot drift apart.

### `initialize()`

The dialect's `initialize()` is where operations, types and attributes are added to the dialect and
where interfaces are attached. A missing `addTypes<>()` produces a crash on first use of the type
with a message about an unregistered type; a missing `addOperations<>()` produces a parse failure.
Both are one-line fixes that are easy to overlook when adding the fifth type to a dialect.

### Layout that scales

A dialect that is going to grow is usually laid out as:

```
include/MyProject/Dialect/MyDialect/IR/
    MyDialect.td          dialect record, includes the rest
    MyDialectOps.td       operations
    MyDialectTypes.td     types
    MyDialectAttrs.td     attributes
    MyDialectInterfaces.td
lib/Dialect/MyDialect/IR/
    MyDialectDialect.cpp  dialect hooks, initialize()
    MyDialectOps.cpp      verifiers, folders, custom parse/print
```

The split matters because TableGen regenerates everything in a file when anything in it changes, so
a monolithic `.td` becomes a build-time bottleneck surprisingly quickly.


---

[← Interfaces](../01-core-ir/07-interfaces.md) · [Index](../README.md) · [Operation Definition Specification (ODS) →](02-operation-definition-specification.md)
