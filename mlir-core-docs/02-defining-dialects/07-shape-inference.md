...
> **Section:** Defining Dialects · document 7 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/ShapeInference/](https://mlir.llvm.org/docs/ShapeInference/) · source [`mlir/docs/ShapeInference.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/ShapeInference.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The first hard problem most new dialects hit. Frontends produce operations with dynamic or unknown
shapes; almost every useful optimization — tiling, vectorization, buffer allocation, fusion — wants
static shapes. Shape inference is the machinery that propagates what is known.

The document is part design discussion and part description of the available interfaces:
`InferTypeOpInterface`, `InferShapedTypeOpInterface`, `ReifyRankedShapedTypeOpInterface`.

**Read first**

- [Interfaces](../01-core-ir/07-interfaces.md)
- [The `Broadcastable` Trait](../01-core-ir/06-trait-broadcastable.md)

**What you should be able to do after this page**

- Distinguish static type inference from runtime shape computation.
- Implement the appropriate inference interface on your operations.
- Decide where in a pipeline shape refinement should run.
...
### Three distinct problems under one name

1. **Type inference at construction.** Given operands and attributes, what are the result types?
   `InferTypeOpInterface`. Runs when the operation is built or parsed, and lets builders and
   assembly formats omit result types.
2. **Shape refinement across the module.** Propagating known shapes forward — and sometimes
   backward — through a function, replacing dynamic dimensions with static ones. A whole-module
   analysis, iterated to fixpoint, usually interleaved with canonicalization because folding exposes
   more static information which enables more refinement.
3. **Runtime shape computation.** Emitting IR that computes shapes at run time, for the cases that
   genuinely cannot be resolved statically. `ReifyRankedShapedTypeOpInterface` and the `shape`
   dialect. Needed for allocation sizes and for shape assertions.

Conflating (2) and (3) is the usual source of confusion when reading this document.

### Practical guidance

**Refinement is iterative, not a single pass.** The productive pipeline shape is
`canonicalize → shape refinement → canonicalize → …` to fixpoint. A single refinement pass leaves
information on the table because folds performed afterwards would have enabled further refinement.

**Static beats dynamic, so push refinement early.** Every dynamic dimension surviving into code
generation costs you: extra branches, unfused loops, dynamic allocation, no vectorization. It is
usually worth substantial effort at the frontend boundary — including specializing on observed input
shapes — to get static shapes before optimization begins.

**Do not let unranked types persist.** An unranked tensor blocks nearly everything. Rank inference
should happen at or immediately after import.

**Assertions are the pragmatic fallback.** When you know a shape but cannot prove it, emitting a
runtime check and then treating the shape as static downstream is a legitimate and widely used
strategy — provided the failure path is well defined.

### The interfaces, concretely

| Interface | Question it answers |
|-----------|--------------------|
| `InferTypeOpInterface` | what are my result types, given operands and attributes? |
| `InferShapedTypeOpInterface` | what are my result shapes, separately from element types? |
| `ReifyRankedShapedTypeOpInterface` | emit IR that computes my result shape at run time |

`InferTypeOpInterface` is the one to implement first: it improves builder ergonomics immediately and
costs almost nothing when the rule is simple. For elementwise operations the
`InferTensorType`/`SameOperandsAndResultType` helpers give it to you without writing the method.

### If you are writing a frontend

Get shape inference working *before* investing in optimization passes. Every optimization is easier
against static shapes, and a pipeline built to tolerate dynamic shapes everywhere is much harder to
retrofit than one that assumes static shapes and handles dynamism at the edges.


---

[← Creating a Dialect: Build Setup](06-creating-a-dialect-build-setup.md) · [Index](../README.md) · [Pass Infrastructure →](../03-passes-and-rewriting/01-pass-infrastructure.md)
