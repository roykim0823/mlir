...
> **Section:** Bindings and Embedding · document 1 of 2  
> **Upstream:** [https://mlir.llvm.org/docs/CAPI/](https://mlir.llvm.org/docs/CAPI/) · source [`mlir/docs/CAPI.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/CAPI.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

C++ has no stable ABI, so a C++ API cannot be used across compiler versions or from other languages.
The C API is the answer: an opaque-handle interface with explicit ownership conventions, covering
contexts, modules, operations, types, attributes, passes and dialect registration.

Read it if you embed MLIR in a host application, write bindings for another language, or use the
Python bindings and want to understand what is underneath them.
...
- Use the C API's naming and ownership conventions correctly.
- Expose your own dialect through the C API.
- Understand what the Python bindings sit on top of.
...
Opaque structs wrapping pointers (`MlirContext`, `MlirOperation`, `MlirType`), consistent naming
(`mlirOperationCreate`, `mlirOperationGetName`), and null-checking via `mlirOperationIsNull`.

Ownership is the part to get right, and it is not uniform:

- **Context-owned**: types, attributes, locations. Uniqued, live as long as the context, never freed
  individually.
- **Explicitly owned**: contexts, modules, pass managers. You destroy them.
- **Parent-owned**: operations inside a module. The parent owns them; destroying the module destroys
  them.

Most memory errors in C API code come from applying the wrong rule. `mlirTypeDestroy` does not exist
because it would be meaningless; `mlirModuleDestroy` very much does.

### Strings

`MlirStringRef` is a pointer and a length, not null-terminated, and generally does not own its data.
Callbacks that produce strings — printing, for instance — pass a `MlirStringRef` valid only for the
duration of the call. Copy it if you need it afterwards. This is the second most common mistake after
ownership.

The printing API is callback-based for exactly this reason: rather than returning an allocated
string whose ownership would be ambiguous, it hands you chunks and lets you decide what to do with
them.

### Extending it for your dialect

Downstream dialects add their own C API following the same conventions, typically one header
declaring the dialect registration plus constructors and accessors for its custom types and
attributes. The upstream dialects' C API headers are the template. Doing this is what lets your
dialect be used from Python.

The macro `MLIR_DECLARE_CAPI_DIALECT_REGISTRATION` generates the registration entry point, which
keeps the boilerplate to one line per dialect.

### Stability

The C API is more stable than the C++ API, not eternally stable. It changes between releases, more
slowly and more visibly. Pin your MLIR version and read the release notes; do not assume a binding
compiled against one major version works with another.


---

[← `mlir-rewrite`](../06-tooling-and-debugging/07-mlir-rewrite.md) · [Index](../README.md) · [MLIR Python Bindings →](02-python-bindings.md)
