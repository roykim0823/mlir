...
> **Section:** Bindings and Embedding · document 2 of 2  
> **Upstream:** [https://mlir.llvm.org/docs/Bindings/Python/](https://mlir.llvm.org/docs/Bindings/Python/) · source [`mlir/docs/Bindings/Python.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Bindings/Python.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The most used non-C++ interface to MLIR, and the one that makes MLIR practical for research and for
integration with machine-learning frameworks. Covers building the bindings, the core API (context,
module, operations, types, attributes), the generated per-dialect operation classes, the pass
manager, the execution engine, and how to add bindings for your own dialect.

Long, and worth reading in full if Python is how you will drive MLIR. The sections on context and
insertion-point management, and on generating bindings for a downstream dialect, save the most time.

**Read first**

- [MLIR C API](01-c-api.md)

**What you should be able to do after this page**

- Build MLIR with Python bindings enabled and import them.
- Construct and transform IR from Python idiomatically.
- Generate and package bindings for your own dialect.
...
For further details, see the documentation of [the IRDL dialect](https://mlir.llvm.org/docs/Dialects/IRDL/).
...
For further information, see [the PDL dialect documentation](https://github.com/llvm/llvm-project/blob/main//docs/Dialects/PDLOps).
...
Context managers do most of the work:
...
Implicit context, location and insertion point are thread-local and scoped. That makes the common
case terse; it also means an operation constructed outside a `with` block raises an error about a
missing context that does not obviously point at the missing `with`.

### Building

Bindings require `-DMLIR_ENABLE_BINDINGS_PYTHON=ON`, a matching Python, and `nanobind`. Version
mismatches between the Python used at build time and at run time produce import errors that are
opaque; check that first when the import fails. `PYTHONPATH` must include the
`tools/mlir/python_packages/mlir_core` directory of your build tree.

### Your own dialect

`declare_mlir_dialect_python_bindings` in CMake generates operation classes from your ODS, which is
the same mechanism upstream dialects use. You supply the C API for any custom types and attributes
([MLIR C API](01-c-api.md)). The result is that your dialect looks exactly like an upstream one from
Python, which is the right outcome.

### Performance

Python is a control layer, not a hot path. Building a large module operation by operation from Python
is slow — each construction crosses the language boundary. The practical pattern is to build a small
amount of IR, or parse it from text, and then run passes in C++ where the real work happens. If you
find yourself constructing hundreds of thousands of operations from Python, move that loop.

Parsing from a string is often the fastest way to get a large module into Python, and it is
frequently overlooked in favour of laborious programmatic construction.

### The execution engine

The bindings expose MLIR's JIT, so you can lower to LLVM and invoke the result from Python directly.
This is what makes the bindings viable for interactive experimentation and for testing numerics
against NumPy — and it is the fastest way to check that a lowering you just wrote actually computes
the right answer.

### Interoperability with array libraries

The bindings support the buffer protocol and DLPack in places, which is what lets you move data
between NumPy arrays and MLIR-managed buffers without copying. If you are building a
research pipeline that alternates between Python numerics and compiled kernels, that is the seam to
understand.


---

[← MLIR C API](01-c-api.md) · [Index](../README.md) · [Side Effects and Speculation →](../08-rationale/01-side-effects-and-speculation.md)
