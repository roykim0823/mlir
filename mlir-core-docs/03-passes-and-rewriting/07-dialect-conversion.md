...
> **Section:** Passes and Rewriting · document 7 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/DialectConversion/](https://mlir.llvm.org/docs/DialectConversion/) · source [`mlir/docs/DialectConversion.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DialectConversion.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The framework for lowering. Where the greedy driver applies patterns until nothing changes, the
conversion driver applies them until the IR is *legal* according to a `ConversionTarget` you declare
— and can roll back if it cannot get there.

Three pieces to understand: the `ConversionTarget` (which operations are legal, illegal, or
dynamically legal), the `TypeConverter` (how types map, and how values are materialized when they
cross a converted/unconverted boundary), and the conversion patterns themselves, which receive
*adapted* operands already in the target type system.

This is the single most error-prone part of MLIR for newcomers, mostly because of type
materialization. Budget time for it.
...
- Set up a partial or full conversion with a correct legality specification.
- Write a type converter and the materializations it needs.
- Diagnose 'failed to legalize operation' and type-mismatch failures.
...
described in the main [Pattern](03-pattern-rewriting.md) documentation. The patterns
...
### Full vs partial, and why partial is the default

**Full conversion** requires every operation to be legal at the end; anything illegal is an error.
**Partial conversion** allows illegal operations to remain.

Partial conversion is the normal mode, because MLIR pipelines lower in stages and intermediate states
are legitimately mixed. Reach for full conversion only at the final step before translation, where
"nothing outside the target dialect survives" really is the requirement.

### The three legality states

| State | Meaning |
|-------|---------|
| Legal | leave it alone |
| Illegal | must be converted, else failure |
| Dynamically legal | a callback decides per operation |

Dynamic legality is what you need for "this function is legal only once its signature types have
been converted" — a rule that cannot be expressed as a static property of the operation. Getting this
wrong gives you either premature success, with operations left unconverted, or an unsatisfiable
target.

### Type conversion is where the difficulty lives

A `TypeConverter` maps source types to target types, and supplies **materializations** — the casts
inserted when a value crosses between converted and unconverted regions:

- **source materialization** — a target-typed value is needed where a source type is expected;
- **target materialization** — a source-typed value is needed where a target type is expected;
- **argument materialization** — for block arguments whose types changed.

The archetypal failure is a missing materialization: the driver has a value of the wrong type, no way
to build a cast, and reports a legalization failure naming an operation which is not actually the
problem. When you see "failed to legalize operation X" and X looks fine, suspect a type at its
boundary rather than the operation.

One-to-many conversions — one source value becoming several target values, as when a struct is
flattened into scalars or a buffer reference into a descriptor's fields — are supported but are the
sharpest edge in the framework. Hand-rolled handling of this case is where subtle bugs accumulate.

### Adaptors

A conversion pattern's `matchAndRewrite` receives an `OpAdaptor` holding the **already-converted**
operands. Use those, not `op.getOperands()` — the latter gives the original, pre-conversion values,
and using them produces IR referencing values from the wrong type system. Almost everyone makes this
mistake once.

```cpp
LogicalResult matchAndRewrite(my::AddOp op, OpAdaptor adaptor,
                              ConversionPatternRewriter &rewriter) const override {
  // adaptor.getLhs() is converted; op.getLhs() is not.
  rewriter.replaceOpWithNewOp<target::AddOp>(op, adaptor.getLhs(), adaptor.getRhs());
  return success();
}
```
...
Changing a function's signature is not a matter of setting its type: the entry block's arguments must
change too, and callers must be updated. `SignatureConversion` and the
`populateFunctionOpInterfaceTypeConversionPattern` helpers exist for this. Hand-rolling it is a
reliable way to produce IR that verifies and then crashes during translation.

### Debugging checklist

1. `--debug-only=dialect-conversion` prints legalization attempts and which patterns were tried.
2. Check the target: is the failing operation marked illegal with no pattern, or legal when you
   expected it to convert?
3. Check the type converter covers every type reachable from the operations you convert, including
   function signatures and block arguments.
4. Check you used the adaptor.
5. Reduce the input until one operation remains. Most conversion bugs are three lines of IR once
   isolated.

### Rollback, and why the rewriter rule is strict here

The driver maintains a rollback log so a failed legalization leaves the IR untouched. This is why
"mutate only through the rewriter" ([Pattern Rewriting](03-pattern-rewriting.md)) is not negotiable
in conversion patterns: a bypassed mutation cannot be rolled back, and the resulting corruption
surfaces long after the pattern that caused it.


---

[← Quickstart: Adding a Graph Rewrite](06-quickstart-adding-a-rewrite.md) · [Index](../README.md) · [Writing Dataflow Analyses →](08-writing-dataflow-analyses.md)
