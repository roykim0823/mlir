# PDLL — The PDL Language

> **Section:** Passes and Rewriting · document 5 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/PDLL/](https://mlir.llvm.org/docs/PDLL/) · source [`mlir/docs/PDLL.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/PDLL.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The most developed of the declarative pattern systems. PDLL is a purpose-built language for writing
rewrite patterns, compiled to the PDL dialect, which can either be compiled to C++ or interpreted at
run time by the PDL bytecode interpreter.

Two things distinguish it. It has an actual language design rather than TableGen's DAG syntax —
constraints, rewrites, named patterns, imports of ODS definitions — and it has an LSP server, so you
get completion, diagnostics and go-to-definition while writing patterns.

Long document; the first third gets you writing patterns and the rest is reference.

**Read first**

- [Pattern Rewriting](03-pattern-rewriting.md)
- [Table-driven Declarative Rewrite Rules (DRR)](04-declarative-rewrite-rules-drr.md)

**What you should be able to do after this page**

- Write and register a PDLL pattern.
- Reuse ODS definitions and native constraints from PDLL.
- Decide whether run-time pattern loading buys you anything.
...
[MLIR Pattern Rewriting](03-pattern-rewriting.md) and
...
[TDRR](04-declarative-rewrite-rules-drr.md), please refer the
relevant documentation before continuing.

Tablegen DRR (TDRR), i.e.
[Table-driven Declarative Rewrite Rules](04-declarative-rewrite-rules-drr.md),
...
[ODS](../02-defining-dialects/02-operation-definition-specification.md) information within that file.
...
Constraint GlobalConstraint(inputValue: Value) {
  ...
  Constraint(value: Value) [{ ... }](https://github.com/llvm/llvm-project/blob/main/mlir/docs/inputValue);
}
```
...
Rewrite GlobalRewrite(inputValue: Value) {
  ...
  Rewrite(value: Value) [{ ... }](https://github.com/llvm/llvm-project/blob/main/mlir/docs/inputValue);
}
```
...
Pattern FoldTransposePair {
  let inner = op<my.transpose>(input: Value);
  HasOneUse(inner);
  let outer = op<my.transpose>(inner);
  replace outer with input;
}
```

Note the ODS import: operation definitions come from the `.td` files you already wrote, so PDLL knows
your operand names and result counts and can diagnose mistakes at pattern-compile time rather than at
C++ compile time.

### The run-time story, and when it matters

PDLL compiles to the PDL dialect, which is IR. That IR can be interpreted by the PDL bytecode
interpreter, meaning patterns can be loaded from a file without rebuilding the compiler.

This is a real capability with a narrow but valuable audience: shipping optimizations as data,
letting users supply target-specific patterns, or A/B testing pattern sets without a rebuild. If none
of those describe your situation, PDLL's value is the language and the tooling, not the dynamism, and
you can compile the patterns in like any other.

The costs of interpretation are a performance hit relative to compiled patterns, and slightly thinner
tooling coverage in places.

### Practical notes

**Native constraints and rewrites** are the C++ escape hatch, declared in PDLL and implemented in
C++. Same principle as DRR's `NativeCodeCall`; same warning about what it means when most of your
pattern is native code.

**Set up the LSP server early** — see
[MLIR Language Server Protocol](../06-tooling-and-debugging/05-language-server-protocol.md). Writing
PDLL without it is substantially less pleasant, and the setup is a few lines of editor configuration.

**Adoption is uneven upstream.** Much of the upstream codebase still uses C++ patterns and DRR, so
you will read more of those than you write of PDLL. That is not an argument against PDLL for new
work, but it does mean fewer examples to copy from.

### Generating C++ from PDLL

`mlir-pdll --output-split-marker` and the `-x=cpp` mode generate the C++ pattern registration, which
is what you use when you do not need run-time loading. The CMake helper
`add_mlir_pdll_library` wires this into the build the same way `add_mlir_dialect` does for ODS.


---

[← Table-driven Declarative Rewrite Rules (DRR)](04-declarative-rewrite-rules-drr.md) · [Index](../README.md) · [Quickstart: Adding a Graph Rewrite →](06-quickstart-adding-a-rewrite.md)
