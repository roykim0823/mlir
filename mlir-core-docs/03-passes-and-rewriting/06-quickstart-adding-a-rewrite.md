# Quickstart: Adding a Graph Rewrite

> **Section:** Passes and Rewriting · document 6 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/Tutorials/QuickstartRewrites/](https://mlir.llvm.org/docs/Tutorials/QuickstartRewrites/) · source [`mlir/docs/Tutorials/QuickstartRewrites.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Tutorials/QuickstartRewrites.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Filed upstream as a tutorial, and placed here because it is the practical consolidation of
everything in sections 02 and 03: define an operation in ODS, write a rewrite pattern for it in DRR,
register it, and test it. Short, concrete, and the fastest way to check that you have understood the
preceding pages.

If the material in the previous five documents felt abstract, do this before continuing.
...
- Go from an idea to a tested, registered rewrite pattern.
- Write a `lit`/`FileCheck` test for a pattern.
...
[Table-driven Operation Definition](../02-defining-dialects/02-operation-definition-specification.md) and
[Declarative Rewrite Rule](04-declarative-rewrite-rules-drr.md) for the detailed explanation
...
[Writing a Pass](01-pass-infrastructure.md) for a general overview and introduction to
...
1. Declare the operation in ODS.
2. Write the pattern, in DRR or C++.
3. Register it — in the operation's `getCanonicalizationPatterns`, or in a pass's pattern set.
4. Write a `.mlir` test with `// RUN:` and `// CHECK:` lines.
5. Run `mlir-opt` by hand on the test input while iterating.

Step 5 is the one to internalise. `mlir-opt --my-pass --debug-only=greedy-rewriter input.mlir` with a
three-line input file is the tightest debugging loop MLIR offers, and it beats reasoning about the
pattern in the abstract every time.

### On test style

Upstream tests are the best available corpus of examples, and they are written to be read: minimal
input, `CHECK` lines that assert the shape of the output rather than every token, and one behaviour
per test function. Copying that style pays off when a test fails a year later and you need to work
out what it was protecting.

`--split-input-file` lets one file hold many independent cases separated by `// -----`, which is the
standard way to keep negative tests — patterns that should *not* fire — next to positive ones.
Negative tests are the ones people skip, and they are what catch an over-eager pattern.

### FileCheck idioms worth knowing

| Directive | Meaning |
|-----------|---------|
| `CHECK:` | must appear, in order |
| `CHECK-NEXT:` | on the immediately following line |
| `CHECK-SAME:` | on the same line as the previous match |
| `CHECK-NOT:` | must not appear before the next positive match |
| `CHECK-DAG:` | a group that may match in any order |
| `CHECK-LABEL:` | resets matching state; use one per function |
| `// CHECK: %[[X:.*]] = my.op` | capture a value name and refer to it later as `%[[X]]` |

`CHECK-LABEL` per function and captured variable names for SSA values are what make tests robust
against unrelated changes — without them, renumbering of `%0`, `%1` breaks tests that were otherwise
fine.


---

[← PDLL — The PDL Language](05-pdll-pattern-language.md) · [Index](../README.md) · [Dialect Conversion →](07-dialect-conversion.md)
