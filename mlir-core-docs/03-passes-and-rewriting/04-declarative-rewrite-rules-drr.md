# Table-driven Declarative Rewrite Rules (DRR)

> **Section:** Passes and Rewriting · document 4 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/DeclarativeRewrites/](https://mlir.llvm.org/docs/DeclarativeRewrites/) · source [`mlir/docs/DeclarativeRewrites.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/DeclarativeRewrites.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The TableGen way to write rewrite patterns: declare a source DAG and a target DAG and let
`mlir-tblgen` generate the C++ `RewritePattern`. For the large class of patterns that are
structurally simple — matching a small operation tree and rebuilding it differently — this is far
less code than the equivalent C++ and considerably harder to get wrong.

Covers the DAG syntax, operand and attribute binding, constraints, multi-result patterns, native code
calls for the parts that cannot be declarative, and the benefit controls.

**Read first**

- [Pattern Rewriting](03-pattern-rewriting.md)
- [Operation Definition Specification (ODS)](../02-defining-dialects/02-operation-definition-specification.md)

**What you should be able to do after this page**

- Write a multi-op DRR pattern with constraints and attribute bindings.
- Escape to C++ with native code calls where the declarative form runs out.
- Choose between DRR, PDLL and a hand-written C++ pattern.
...
[Op Definition Specification](../02-defining-dialects/02-operation-definition-specification.md) (ODS), this is achieved via
...
[Quickstart tutorial to adding MLIR graph rewrite](06-quickstart-adding-a-rewrite.md)
for the latter.

Given that declarative rewrite rules depend on op definition specification, this
manual assumes knowledge of the [ODS](../02-defining-dialects/02-operation-definition-specification.md) doc.
...
```tablegen
def FoldTransposePair : Pat<
    (MyDialect_TransposeOp (MyDialect_TransposeOp $x, $p1), $p2),
    (replaceWithValue $x),
    [(AreInverse $p1, $p2)]>;
```

Source DAG, target DAG, optional list of constraints. Names bound with `$` in the source are
available in the target and in the constraints.

### The bits people trip over

**Multi-result and multi-op targets** need `Pattern` (with a list of target DAGs) rather than `Pat`,
and results are bound positionally. The error messages when this is wrong come from generated C++ and
are not self-explanatory, which is why the corresponding section of the page is worth reading before
you need it rather than after.

**`NativeCodeCall` is the escape hatch,** and it is fine to use — most non-trivial patterns have one
somewhere. But a DRR rule that is mostly native calls is a C++ pattern wearing a costume; at that
point write the C++ and keep it readable.

**Constraints run at match time, in order.** Put the cheap discriminating ones first.

**Attribute binding differs from operand binding.** An attribute matched in the source can be used
directly in the target, transformed by a `NativeCodeCall`, or constrained — but it cannot be matched
structurally the way an operand tree can. Patterns needing to inspect attribute *contents* usually
end up with a native constraint.

**Locations.** The generated pattern should attach a sensible location to what it creates, or
diagnostics downstream point at the wrong source line and you will not enjoy tracking it back.

### DRR vs PDLL vs C++

| | DRR | PDLL | C++ |
|---|---|---|---|
| Lives in | `.td` | `.pdll` | `.cpp` |
| Expressiveness | structural matching, native escapes | fuller matching language, better tooling | anything |
| Compiled or interpreted | compiled | either | compiled |
| Best for | simple, numerous algebraic patterns | large declarative pattern sets, dynamic loading | complex analysis-driven rewrites |

DRR remains the pragmatic default for a downstream dialect with a lot of small algebraic
simplifications, chiefly because it lives in the `.td` file you already have.
[PDLL](05-pdll-pattern-language.md) is the more capable system with real tooling behind it; whether
migrating is worth it depends on how many patterns you have and whether you need run-time loading.

### Where DRR rules are registered

`mlir-tblgen --gen-rewriters` produces a `populateWithGenerated(patterns)` function. Call it from
your pass, or from `getCanonicalizationPatterns`. Forgetting that call is the most common reason a
freshly written DRR rule appears to do nothing.


---

[← Pattern Rewriting: Generic DAG-to-DAG Rewriting](03-pattern-rewriting.md) · [Index](../README.md) · [PDLL — The PDL Language →](05-pdll-pattern-language.md)
