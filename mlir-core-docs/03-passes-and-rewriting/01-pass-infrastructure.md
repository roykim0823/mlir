...
> **Section:** Passes and Rewriting · document 1 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/PassManagement/](https://mlir.llvm.org/docs/PassManagement/) · source [`mlir/docs/PassManagement.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/PassManagement.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The container for all transformation work. Covers writing a pass, declaring it in TableGen with
options and statistics, nesting pass managers over the operation hierarchy, the analysis manager and
its caching and invalidation rules, multithreaded execution, dynamic pipelines, textual pipeline
specification, and instrumentation including crash reproducers.

Long, but the parts you need early are short: how to write a pass, how to nest pass managers, and
how the textual pipeline syntax works. Read the threading section before you put any mutable state
in a pass — that rule is easy to violate accidentally and produces nondeterministic failures.
...
- Write, declare and register a pass with options and statistics.
- Nest pass managers correctly and explain why nesting affects parallelism.
- Use analyses without recomputing them, and invalidate them correctly.
- Produce a crash reproducer for a failing pipeline.
...
See [MLIR specification](../01-core-ir/01-language-reference.md) for more information about MLIR and its
core aspects, such as the IR structure and operations.

See [MLIR Rewrites](06-quickstart-adding-a-rewrite.md) for a quick start on graph
...
[operation](../01-core-ir/01-language-reference.md#operations). As such, the pass manager is designed to
...
[Canonicalization](https://mlir.llvm.org/docs/Passes/#-canonicalize) and [Common Sub-Expression Elimination](https://mlir.llvm.org/docs/Passes/#-cse).
...
such example is when using [`PDL`-based](https://mlir.llvm.org/docs/Dialects/PDLOps/)
[patterns](03-pattern-rewriting.md), which are compiled into a bytecode during
...
*   Must be registered and marked
    [`IsolatedFromAbove`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Traits#isolatedfromabove).
...
[structural](../01-core-ir/03-understanding-the-ir-structure.md) nesting within
[Regions](../01-core-ir/01-language-reference.md#regions) of the IR.
...
[Inliner Pass](https://mlir.llvm.org/docs/Passes/#-inline) may want to run
...
[operations](../02-defining-dialects/02-operation-definition-specification.md). This specification simplifies several mechanisms
...
passes can be generated. See [Passes.md](https://mlir.llvm.org/docs/Passes/) for example output of real
MLIR passes.
...
### Nesting is the concept to get right first

A pass manager runs over a specific operation type. `PassManager pm(ctx)` operates on the top-level
operation; `pm.nest<func::FuncOp>()` creates a nested manager whose passes run on each function.
Textually:

```
builtin.module(canonicalize, func.func(cse, my-pass), symbol-dce)
```

`cse` and `my-pass` run on every function; `canonicalize` and `symbol-dce` run on the module.

This is not merely organisational. Passes nested under an `IsolatedFromAbove` operation can run
**in parallel across instances**, because they provably cannot interfere. That is where MLIR's
compile-time scalability comes from, and it is why the framework enforces the rules below.

A frequent early confusion: a pass registered as operating on functions cannot be added to a module
pass manager directly. The pipeline string must nest it. The error message says so, but it is easy
to misread as the pass not existing.

### The threading rules

A pass nested under an isolated operation must not:

- mutate anything outside the operation it was given, including sibling functions;
- keep mutable state across `runOnOperation()` invocations — the same pass object may be used for
  many operations concurrently;
- assume any ordering between instances.

Violations produce data races manifesting as nondeterministic verifier failures or crashes, often
only on large inputs. If you must accumulate results, use per-operation state and merge afterwards,
or mark the pass as requiring a single thread. `--mlir-disable-threading` is the first thing to try
when a pipeline fails intermittently — if it fixes it, you have a threading bug, not a logic bug.

### Analyses

`getAnalysis<T>()` computes and caches; `getCachedAnalysis<T>()` returns it only if already present;
`markAllAnalysesPreserved()` and `markAnalysesPreserved<T>()` declare what survived. The default is
that a pass invalidates everything, which is safe and often wasteful. Declaring preservation
accurately is a cheap and frequently overlooked compile-time win in a long pipeline.

An analysis is any class with the right constructor; there is no registration. Analyses are keyed by
type and by the operation they were computed on, so an analysis over a function is not shared with
its module.

### Options, statistics, and pipeline text

Options declared in ODS become both C++ fields and textual pipeline options:
`my-pass{tile-size=32 aggressive=true}`. Statistics declared the same way are reported with
`--mlir-pass-statistics`. Getting these declarations right is what makes a pass usable from
`mlir-opt` without a custom driver, which in turn is what makes it testable with `lit`.

### Instrumentation, and the flag that saves the most time

| Flag | Use |
|------|-----|
| `--mlir-print-ir-after-all` | see the IR after every pass |
| `--mlir-print-ir-after-failure` | only when something fails |
| `--mlir-print-ir-before=my-pass` | before a specific pass |
| `--mlir-timing` | per-pass timing |
| `--mlir-pass-statistics` | declared statistics |
| `--mlir-pass-pipeline-crash-reproducer=repro.mlir` | dump a minimal reproducer on crash |
| `--mlir-disable-threading` | serialize, for determinism while debugging |

The crash reproducer is the one to remember. It writes the IR as it was on entry to the failing pass,
plus the pipeline needed to re-run just that pass, turning "it crashed somewhere in a forty-pass
pipeline on a large model" into a single small file. Combine it with
[`mlir-reduce`](../06-tooling-and-debugging/06-mlir-reduce.md) to shrink further.

### Pass or pattern?

Write a **pattern** for a local rewrite that composes with others and should run to fixpoint with
the rest of canonicalization. Write a **pass** when you need ordering control, a whole-module
analysis, a specific driver, or a transformation that is not a local rewrite at all — bufferization,
for instance. A pass that is nothing but a fixpoint over one pattern is usually better expressed as a
canonicalization pattern on the relevant operation.


---

[← Shape Inference](../02-defining-dialects/07-shape-inference.md) · [Index](../README.md) · [Operation Canonicalization →](02-operation-canonicalization.md)
