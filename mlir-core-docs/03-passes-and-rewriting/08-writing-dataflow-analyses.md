...
> **Section:** Passes and Rewriting · document 8 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/Tutorials/DataFlowAnalysis/](https://mlir.llvm.org/docs/Tutorials/DataFlowAnalysis/) · source [`mlir/docs/Tutorials/DataFlowAnalysis.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Tutorials/DataFlowAnalysis.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Filed upstream as a tutorial; placed here because it is the natural end of the transformation
section. Patterns are local by construction — they see a subgraph. When a transformation needs
information that is not locally visible (is this value always positive? can this pointer alias that
one? is this branch dead?), you need a dataflow analysis.

Covers MLIR's dataflow framework: lattices, transfer functions, the sparse analysis (state on values)
and dense analysis (state at program points), and how the solver handles regions and calls.
...
- Define a lattice and a transfer function for a simple analysis.
- Choose between sparse and dense formulations.
- Understand how the solver handles regions, branches and calls.
...
**Sparse** attaches state to SSA *values* and propagates along def-use edges. Use it when the
property is a property of a value: constant-ness, sign, integer range, known bits. Cheap, because it
only visits real dependencies.

**Dense** attaches state to *program points* and propagates along control flow. Use it when the
property is about the program state at a location: what memory holds, which buffers are live, what
locks are held. More expensive, and the right tool for anything memory-related.

Picking sparse when you need dense is a common early mistake — it produces an analysis that is
correct on straight-line code and silently wrong across control flow.

### The pieces you implement

1. **Lattice element** — the value being propagated, with a join and a defined top and bottom. Join
   must be commutative, associative and monotone.
2. **Transfer function** — given input state and an operation, produce output state.
3. **Initial state** — entry conditions, and what unknown inputs mean.

The framework supplies the solver, the worklist and the fixpoint iteration.

### Termination

The lattice must have finite height along any ascending chain, or the solver will not terminate.
Integer ranges are the standard example: the naive lattice is infinite, so a widening operator is
required after a few iterations. If your analysis hangs on a loop, this is the first thing to check.

### Interfaces the solver relies on

`RegionBranchOpInterface` and `BranchOpInterface` tell the solver how control flows through and
between regions; `CallOpInterface` handles interprocedural edges. If your operations do not implement
these, the solver has to assume the worst — which is sound but produces top everywhere, and the
analysis appears to "not work" when in fact it is being maximally conservative. Another case where
attaching the right interfaces is the actual fix.

### Before writing one

Check whether an existing analysis covers it. Integer range analysis, sparse constant propagation,
liveness, and the alias and memory-effect infrastructure all exist upstream and are interface-driven,
so they may already work on your dialect. Writing a dataflow analysis is a real investment, and the
framework's value is highest when the property genuinely is novel.

### Consuming the results

The solver stores lattice state keyed by value or program point; a pass queries it after running the
solver and then rewrites accordingly. Note that the analysis does not mutate anything — keeping
analysis and transformation separate is what lets the same analysis serve several passes, and it is
worth preserving that separation in your own code even when a fused version would be shorter.


---

[← Dialect Conversion](07-dialect-conversion.md) · [Index](../README.md) · [Bufferization →](../04-memory-and-lowering/01-bufferization.md)
