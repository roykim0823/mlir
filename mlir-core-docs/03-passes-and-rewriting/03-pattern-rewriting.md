...
> **Section:** Passes and Rewriting · document 3 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/PatternRewriter/](https://mlir.llvm.org/docs/PatternRewriter/) · source [`mlir/docs/PatternRewriter.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/PatternRewriter.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The engine under canonicalization, dialect conversion, DRR and PDL. Defines `RewritePattern` and
`OpRewritePattern`, the `PatternRewriter` API you must use for all mutation, pattern benefit and
ordering, and the drivers that apply patterns — principally the greedy fixpoint driver.

This is the most-used API in day-to-day MLIR work after the IR walkers, and the rules about mutating
only through the rewriter are not optional: the conversion driver relies on them to be able to roll
back.

**Read first**

- [Understanding the IR Structure](../01-core-ir/03-understanding-the-ir-structure.md)
- [Operation Canonicalization](02-operation-canonicalization.md)

**What you should be able to do after this page**

- Write a `matchAndRewrite` that is correct under the rewriter's notification rules.
- Choose and configure a driver, and set its convergence limits.
- Debug a pattern that does not fire, or fires forever.
...
[Generic DAG Rewriter Rationale](../08-rationale/03-generic-dag-rewriter-rationale.md).
...
driver can be found [here](07-dialect-conversion.md).
...
Note: This driver is the one used by the [canonicalization](02-operation-canonicalization.md)
[pass](https://mlir.llvm.org/docs/Passes/#-canonicalize) in MLIR.
...
**All IR mutation inside a pattern goes through the `PatternRewriter`.** Not `op->erase()`, not
`op->setOperand()`, not a bare `builder.create<>()` — use `rewriter.eraseOp()`,
`rewriter.modifyOpInPlace()`, `rewriter.create<>()`, `rewriter.replaceOp()`.

The reason is that the rewriter notifies the driver of every change. The greedy driver uses those
notifications to re-add affected operations to its worklist; the conversion driver uses them to
maintain a rollback log. Bypassing the rewriter gives you IR that looks correct after the pattern but
leaves the driver's bookkeeping wrong, producing missed rewrites, stale worklists, or crashes during
rollback that appear far from the actual mistake.

For in-place modification specifically, the required shape is:

```cpp
rewriter.modifyOpInPlace(op, [&]{ op.setSomething(...); });
```

`startOpModification` / `finalizeOpModification` is the explicit form when the modification cannot be
expressed as a single lambda.

### Anatomy of a pattern

```cpp
struct FoldTransposePair : OpRewritePattern<my::TransposeOp> {
  using OpRewritePattern::OpRewritePattern;
  LogicalResult matchAndRewrite(my::TransposeOp op,
                                PatternRewriter &rewriter) const override {
    auto inner = op.getInput().getDefiningOp<my::TransposeOp>();
    if (!inner || !isInverse(op, inner))
      return failure();                       // no mutation before this point
...
`failure()` means "did not apply" and is not an error. Perform no mutation before you are certain
you will return `success()` — the separate `match`/`rewrite` split exists to make that discipline
structural, though `matchAndRewrite` is now the common form.

### Benefit and ordering

Patterns carry a benefit; higher benefit is tried first, ties broken by an unspecified but
deterministic order. Do not encode a phase ordering in benefits — if pattern A must run before
pattern B, that is two pattern sets and two driver invocations, not two benefit values. Benefit
expresses "this is a better rewrite of the same thing", not "do this first".

### The greedy driver, and its limits

`applyPatternsGreedily` walks operations, applies patterns to fixpoint, and folds along the way.
Configuration worth knowing: `maxIterations`, `maxNumRewrites`, traversal direction, whether to fold,
and whether to enforce that the region converged.

Failure to converge is reported as hitting the iteration limit. Nearly always this means two patterns
undo each other ([Canonicalization](02-operation-canonicalization.md)).
`--debug-only=greedy-rewriter` shows the firing sequence and makes the cycle obvious.

The greedy driver is *not* the right tool for lowering between dialects: it has no notion of
legality, so a partially-lowered state is indistinguishable from a finished one. That is what the
conversion driver ([Dialect Conversion](07-dialect-conversion.md)) is for.

### Why a pattern does not fire — a checklist

1. Is it actually in the pattern set? Registration is the most common miss.
2. Does the root operation name match? Watch for unregistered-dialect typos.
3. Does an earlier `return failure()` fire? Add a debug print, or use
   `--debug-only=pattern-application`.
4. Is another pattern with higher benefit rewriting the root first?
5. Is the driver reaching the operation at all — is it nested inside something the pass does not
   walk into?

### Listeners

`RewriterBase::Listener` lets you observe insertions, replacements and erasures. This is a practical
debugging tool when you need to know which pattern touched an operation, and it is the mechanism
behind the pattern-logging infrastructure.


---

[← Operation Canonicalization](02-operation-canonicalization.md) · [Index](../README.md) · [Table-driven Declarative Rewrite Rules (DRR) →](04-declarative-rewrite-rules-drr.md)
