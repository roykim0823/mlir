...
> **Section:** Passes and Rewriting · document 2 of 8  
> **Upstream:** [https://mlir.llvm.org/docs/Canonicalization/](https://mlir.llvm.org/docs/Canonicalization/) · source [`mlir/docs/Canonicalization.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Canonicalization.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Canonicalization is MLIR's normalisation phase: it puts IR into a standard form so that later passes
have fewer shapes to match. It is not "optimization" — the distinction is enforced socially and it
matters. A canonicalization must be unconditionally beneficial, target-independent, and terminating.

Read this before writing patterns, because the criteria here decide *where* your rewrite belongs.
Putting a profitability-dependent transformation into canonicalization is a design error that
manifests as passes fighting each other and as fixpoint loops that never converge.

**Read first**

- [Pass Infrastructure](01-pass-infrastructure.md)

**What you should be able to do after this page**

- Decide whether a rewrite belongs in canonicalization, a folder, or a separate pass.
- Register canonicalization patterns for your operations.
- Diagnose a non-terminating canonicalization.
...
[Declarative Rewrite Rules](04-declarative-rewrite-rules-drr.md). The pattern rewrite
...
In [ODS](../02-defining-dialects/02-operation-definition-specification.md), an operation can set the `hasCanonicalizer` bit or
...
See the [quickstart guide](06-quickstart-adding-a-rewrite.md) for information on
...
[dialect conversion infrastructure](07-dialect-conversion.md) as a legalization
...
In [ODS](../02-defining-dialects/02-operation-definition-specification.md), an operation can set the `hasFolder` bit to generate
...
In [ODS](../02-defining-dialects/01-defining-dialects.md), a dialect can set the `hasConstantMaterializer` bit
...
### The three criteria, and what violates them

**Unconditionally beneficial.** If a transformation is a win on one target and a loss on another, or
depends on a cost model, it is not a canonicalization. Loop unrolling, vectorization and tiling all
fail this test.

**Target-independent.** Canonicalization runs at every abstraction level and knows nothing about the
eventual target.

**Terminating.** The classic failure: two patterns that undo each other. `A → B` in one operation's
canonicalizer and `B → A` in another's produces an infinite loop, which the greedy driver reports as
hitting its iteration limit rather than as the logic error it is. Establish a direction — fewer
operations, simpler operations, constants toward one side — and make every pattern move strictly in
that direction.

### Where to put a rewrite

| Rewrite | Home |
|---------|------|
| `x + 0 → x`, constant evaluation | `fold()` |
| `transpose(transpose(x)) → x` | canonicalization pattern |
| tiling, unrolling, vectorization | dedicated pass with a cost model |
| lowering to another dialect | conversion pattern ([Dialect Conversion](07-dialect-conversion.md)) |

Prefer `fold()` when it suffices: it is cheaper, it cannot loop, and it runs in more contexts —
including inside the pattern rewriter itself, so folds compose with everything.

### Conventions worth following

Canonicalization conventionally moves constants to the right for commutative operations (the
`Commutative` trait plus the canonicalizer does this for you), sinks constants toward their uses, and
eliminates no-op casts and identity operations. Following the same conventions in your dialect means
your patterns compose with upstream ones instead of fighting them.

### Registration and debugging

`let hasCanonicalizer = 1` for a method that adds several patterns; `let hasCanonicalizeMethod = 1`
for a single pattern written as a static method. Both are picked up by the `canonicalize` pass
automatically.

When canonicalization misbehaves, `--debug-only=greedy-rewriter` prints which patterns fire on which
operations, which identifies a fighting pair in seconds. It is much faster than reasoning about it.

`canonicalize` also accepts options — `top-down`, `max-iterations`, `max-num-rewrites`, and
`test-convergence` — which are useful both for diagnosing non-convergence and for bounding
compile time on very large inputs.

### A note on what canonicalization is for

Its purpose is to reduce the number of distinct shapes downstream passes must handle. That framing
resolves most arguments about whether something belongs here: if it makes the IR *more* uniform, it
is a candidate; if it makes the IR *faster* but not more uniform, it is an optimization and belongs
in its own pass.


---

[← Pass Infrastructure](01-pass-infrastructure.md) · [Index](../README.md) · [Pattern Rewriting: Generic DAG-to-DAG Rewriting →](03-pattern-rewriting.md)
