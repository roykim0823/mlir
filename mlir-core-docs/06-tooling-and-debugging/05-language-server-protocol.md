...
> **Section:** Tooling and Debugging · document 5 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Tools/MLIRLSP/](https://mlir.llvm.org/docs/Tools/MLIRLSP/) · source [`mlir/docs/Tools/MLIRLSP.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Tools/MLIRLSP.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Three language servers: `mlir-lsp-server` for `.mlir`, `mlir-pdll-lsp-server` for PDLL, and
`tblgen-lsp-server` for TableGen. They provide completion, diagnostics, hover documentation and
go-to-definition in your editor.

Set these up. The `.td` and `.pdll` servers in particular turn TableGen from a language where you
discover mistakes at build time into one where you see them as you type, and the `.mlir` server gives
you hover documentation for every operation in your dialect from its ODS `description`.
...
- Configure editor support for MLIR, PDLL and TableGen files.
- Make your own dialect's documentation available in the editor.
...
operation results and block arguments), [Symbols](../01-core-ir/04-symbols-and-symbol-tables.md),
and Blocks.
...
The MLIR server knows about the dialects linked into it. If you build a server binary that registers
your dialect, you get hover text — the `description` from your ODS definition — completion of your
operation names, and immediate parse diagnostics on your custom assembly formats. That last one alone
repays the setup: assembly format errors are otherwise found by running `mlir-opt` on a test.

There is a direct incentive here worth noticing: the `summary` and `description` you write in ODS are
not documentation nobody reads, they are the text that appears in your users' editors. That is a
better argument for writing them well than any style guide.

### Multiple servers

They are separate binaries for separate file types. Editors are configured per language, so all three
can be active simultaneously. For a downstream project, building your own `mlir-lsp-server`
equivalent with your dialects registered is the same few lines as building your own `mlir-opt`.

### Features worth knowing about

Beyond completion and hover: go-to-definition on SSA values and symbols, navigation of symbol
references, and the ability to view an operation in its generic form. The last is more useful than it
sounds when reading an unfamiliar dump full of custom assembly formats you do not know.

### If the server is not working

The usual causes are the binary not being on `PATH`, the editor not associating `.mlir` with the
right language id, or the server binary not having your dialect registered — in which case it works
for upstream operations and silently fails to recognise yours.


---

[← Remark Infrastructure](04-remark-infrastructure.md) · [Index](../README.md) · [`mlir-reduce` →](06-mlir-reduce.md)
