...
> **Section:** Tooling and Debugging · document 2 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Diagnostics/](https://mlir.llvm.org/docs/Diagnostics/) · source [`mlir/docs/Diagnostics.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Diagnostics.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

How MLIR reports problems: `emitError`/`emitWarning`/`emitRemark`, attached notes, the location
system that ties diagnostics back to source, diagnostic handlers, and the scoped handlers that make
diagnostics testable.

Read it when you write your first verifier or pass that can fail. Diagnostic quality is most of the
user experience of a compiler, and MLIR gives you good tools — the location system in particular is
richer than most compilers', and it is underused.
...
- Emit a diagnostic with useful location information and attached notes.
- Preserve locations through a transformation.
- Write tests that assert on expected diagnostics.
...
See [MLIR specification](../01-core-ir/01-language-reference.md) for more information about MLIR, the
...
[builtin dialect](https://mlir.llvm.org/docs/Dialects/Builtin/) provides several different location
...
diagnostics in MLIR. [`operation`](../01-core-ir/01-language-reference.md#operations) provides utility
...
### Locations are richer than a line number

| Location | Meaning |
|----------|---------|
| `FileLineColLoc` | a source position |
| `NameLoc` | a named entity, e.g. a model layer name |
| `CallSiteLoc` | callee location plus the call site — inlining chains |
| `FusedLoc` | several locations combined — the result of fusing operations |
| `OpaqueLoc` | a pointer into a frontend's own AST |
| `UnknownLoc` | no information |

`FusedLoc` matters more than it looks. When a pattern replaces three operations with one, fusing
their locations means a later diagnostic can point at all three sources. Preserving locations through
rewrites is cheap and it is what makes errors from the bottom of a pipeline traceable back to a line
of the user's program. Dropping them — passing `UnknownLoc` because it was convenient — is a small
act with a large downstream cost.

`NameLoc` is the one to reach for when importing a machine-learning model: naming each operation
after the layer it came from means every subsequent diagnostic and every profile is legible in the
user's own vocabulary.

Use `--mlir-print-debuginfo` to see locations in a dump when tracking down where an operation
originated.

### Writing a good diagnostic

```cpp
return op->emitOpError("expected operand rank to match result rank")
       << ", but got " << inRank << " and " << outRank;
```

`emitOpError` prefixes the operation name and attaches its location. Attach a `.attachNote(loc)`
pointing at the *other* entity involved when a constraint relates two things — that is the difference
between "these do not match" and showing the user both.

### Testing diagnostics

`--verify-diagnostics` turns expected diagnostics into assertions:

```mlir
// expected-error @+1 {{expected operand rank to match result rank}}
%0 = my.op %arg : tensor<4xf32> -> tensor<4x4xf32>
```

The test fails if the diagnostic is missing, if an unexpected one appears, or if it is on the wrong
line. Every verifier you write should have such a test; they are cheap and they catch diagnostic
regressions that are otherwise invisible.

`@+1`, `@-1` and `@below`/`@above` control which line the diagnostic is expected on, and
`expected-error@unknown` handles diagnostics with no location.

### Handlers

`ScopedDiagnosticHandler` lets you intercept diagnostics — to collect them, to suppress them during
speculative work, or to reformat them for a host application. `SourceMgrDiagnosticHandler` produces
the caret-and-snippet output you see from `mlir-opt`. If you embed MLIR in a larger tool, installing
your own handler is how you get diagnostics into that tool's error reporting.

Note that diagnostics are emitted from potentially many threads. A handler that accumulates into
shared state needs its own locking, and the framework does not provide it for you.


---

[← Using `mlir-opt`](01-using-mlir-opt.md) · [Index](../README.md) · [Action: Tracing and Debugging MLIR-based Compilers →](03-action-tracing.md)
