...
> **Section:** Rationale and Design History · document 6 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Rationale/MLIRForGraphAlgorithms/](https://mlir.llvm.org/docs/Rationale/MLIRForGraphAlgorithms/) · source [`mlir/docs/Rationale/MLIRForGraphAlgorithms.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Rationale/MLIRForGraphAlgorithms.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Written to argue that an established machine-learning framework could adopt MLIR incrementally rather
than through a rewrite, and to explain what it would gain: a common representation for graph
transformations, verification, better debugging, and reusable infrastructure.

Read it if you face the same question in your own organisation. The argument for *incremental*
adoption, and the specific description of what an ad-hoc graph transformation system costs, is more
broadly applicable than its original framing suggests.
...
- Make the case for incremental MLIR adoption in an existing system.
- Recognise the failure modes of ad-hoc graph rewriting infrastructure.
...
well as [a specification](../01-core-ir/01-language-reference.md) for that format - built just like any
...
feeds the input into the [canonicalize pass](../03-passes-and-rewriting/02-operation-canonicalization.md), and checks
...
The [MLIR Tensor Type](https://mlir.llvm.org/docs/Dialects/Builtin/#rankedtensortype) directly
...
[general rewriting infrastructure](03-generic-dag-rewriter-rationale.md) for
...
Framework graph transformation systems tend to accumulate the same difficulties: no verification, so
invalid graphs are produced and detected far away; no shared pattern infrastructure, so every
transformation reimplements matching; poor debuggability, with no way to print the graph between
steps or attribute a change to a transformation; and no location tracking, so errors cannot be traced
to source.

Each is individually survivable and collectively they make a system nobody wants to modify.

### The incremental argument

The path proposed is not a rewrite: represent the graph in MLIR, convert at the boundaries, move one
transformation at a time, and keep the existing system running throughout. This works because MLIR's
dialect mechanism lets you model the existing representation *faithfully*, including its oddities,
rather than requiring you to first clean it up.

That is the transferable insight. A dialect that mirrors your current representation exactly is a
legitimate and often correct first step, even though it captures none of MLIR's benefits by itself —
it is what makes everything after it incremental.

### Historical note

The argument was successful; several major framework compilers followed roughly this path. Reading it
now is reading a proposal whose outcome is known, which makes it a useful check on whether the
reasoning was actually sound or merely persuasive.

### If you are making this case internally

The two arguments that tend to land are the ones this document leads with: verification, because
"invalid graphs are caught at the point they are created rather than three transformations later" is
concrete and measurable; and debuggability, because being able to print and diff the IR between every
step is something engineers feel the absence of immediately.


---

[← Linalg Dialect Rationale: The Case For Compiler-Friendly Custom Operations](05-structured-ops-rationale.md) · [Index](../README.md) · [Usage of `const` in MLIR for Core IR Types →](07-usage-of-const.md)
