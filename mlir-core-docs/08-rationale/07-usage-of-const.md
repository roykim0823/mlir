...
> **Section:** Rationale and Design History · document 7 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Rationale/UsageOfConst/](https://mlir.llvm.org/docs/Rationale/UsageOfConst/) · source [`mlir/docs/Rationale/UsageOfConst.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Rationale/UsageOfConst.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

A narrow C++ design document explaining why MLIR's core IR types deliberately do not follow the usual
`const`-correctness conventions, and what the alternative is.

Short and specific. Read it if you write C++ against the MLIR APIs and have wondered why
`Operation *` is passed around rather than `const Operation *`, or if you are tempted to add `const`
in a patch and want to know why reviewers will object.
...
- Understand why `const` is avoided on core IR types.
- Express immutability intent in the way MLIR actually does.
...
Values, types and attributes are immutable and uniqued already, so `const` on them adds nothing.
Operations are mutable, but the pervasive use of handle types wrapping `Operation *` means `const`
would propagate in ways more confusing than clarifying — a `const` op class handle would prevent
mutating the handle, not the operation, which is precisely the wrong guarantee.

Rather than fight this, MLIR does not use `const` on core IR types and expresses intent through API
design and naming instead.

### The general point

This is a specific instance of a broader observation: `const` in C++ propagates through pointer and
handle types in ways that do not always match the ownership semantics you intended, and a codebase
built around lightweight handles to shared mutable structures is one where it tends to mislead.

Whether you agree or not, the convention is established, consistently applied, and enforced in
review — so if you contribute to MLIR, follow it. Note that `const` is still used normally elsewhere:
on local variables, on references to containers, on methods of non-IR classes. The exemption is
specifically for the core IR handle types.


---

[← MLIR: Incremental Application to Graph Algorithms in ML Frameworks](06-mlir-for-graph-algorithms.md) · [Index](../README.md) · [MLIR Release Notes →](../09-appendix/01-release-notes.md)
