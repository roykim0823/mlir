...
> **Section:** Rationale and Design History · document 5 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Rationale/RationaleLinalgDialect/](https://mlir.llvm.org/docs/Rationale/RationaleLinalgDialect/) · source [`mlir/docs/Rationale/RationaleLinalgDialect.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Rationale/RationaleLinalgDialect.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The argument behind MLIR's structured operations, and the most intellectually substantial document in
the rationale set. It surveys prior approaches to tensor compilation — Halide, TVM, polyhedral
compilers, library-based approaches — and argues for representing operations in a form that is
*designed to be transformed* rather than designed to be executed.

If you are building a code generator, this is the document that most changes how you think about
operation design. The core claim — that an operation should carry the information a compiler needs to
transform it, and that this is a design constraint on your IR rather than a property to be recovered
by analysis — generalises well beyond its subject.

**Read first**

- [MLIR Rationale](02-mlir-rationale.md)

**What you should be able to do after this page**

- Explain why structured operations are the design they are.
- Apply 'transformability as a design constraint' to your own dialect.
...
[Regions](../01-core-ir/01-language-reference.md#regions),
[OpInterfaces](../01-core-ir/07-interfaces.md),
[ODS](../02-defining-dialects/02-operation-definition-specification.md) and
[Declarative Rewrite Rules](../03-passes-and-rewriting/04-declarative-rewrite-rules-drr.md)
...
[regions](../01-core-ir/01-language-reference.md#regions). It also has the
...
Several [design questions](https://mlir.llvm.org/docs/Dialects/Linalg/#open_issues) remain
...
(i.e. [DRR](../03-passes-and-rewriting/04-declarative-rewrite-rules-drr.md)).
...
implications on the
[semantics of Linalg Ops](https://mlir.llvm.org/docs/Dialects/Linalg/#linalg_ops) and their
...
[Linalg Dialect](https://mlir.llvm.org/docs/Dialects/Linalg/) and its future evolutions aspire
...
### Background: what a structured operation is

The `linalg` dialect (documented at <https://mlir.llvm.org/docs/Dialects/Linalg/>, outside this
collection's scope) represents an operation not as an explicit loop nest but as data: a set of
*indexing maps* saying how each operand is accessed from a point in the iteration space, a list of
*iterator types* saying what each loop dimension means (parallel, reduction, window), and a small
region computing one element.

A matrix multiplication in that form is an iteration space `(m, n, k)`, three maps
`(m,k)`, `(k,n)`, `(m,n)`, iterator types `[parallel, parallel, reduction]`, and a body that
multiplies and accumulates. Nothing about the representation is specific to matrix multiplication —
a convolution or an elementwise map is the same operation with different maps.

### The central argument

Two extremes. A **library** approach gives you an opaque call: fast if a good implementation exists,
useless to the compiler, and unable to fuse with its neighbours. A **fully lowered loop nest** gives
the compiler everything: complete freedom, and the need to rediscover, by analysis, the structure
that was thrown away.

Structured operations occupy the middle. The operation retains its structure as *data*, so a
transformation reads it rather than inferring it. Tiling does not need to prove anything; it reads
the maps.

This is the "compiler-friendly" of the title, and it is a general design principle: when you design
an operation, ask what a transformation would need to know about it, and put that in the operation.

### Consequences worth noticing

- **Generic transformations become possible.** One tiling implementation for all structured
  operations.
- **Analysis burden drops.** No loop-nest recognition, no idiom matching.
- **Composition improves.** Fusion becomes map composition rather than pattern matching.
- **The cost is expressiveness.** Not everything fits the structured form, and forcing something that
  does not fit produces awkward operations. Knowing when to leave the form is part of using it.

### Applying it to an accelerator dialect

The practical version of this advice: before defining an operation for your hardware, list the
transformations you will want to apply to it — tiling to fit a scratchpad, fusing with a neighbour,
splitting a reduction across processing elements — and check that the operation as defined carries
enough information to perform each without analysis. If it does not, add the information rather than
planning to recover it later.

### Where it sits historically

Written before explicit transformation scheduling existed in MLIR, so the scheduling story it
describes has since been substantially developed. The representational argument is unchanged and
remains the best statement of why the structured-operations layer exists.


---

[← The Case for a Simplified Polyhedral Form](04-simplified-polyhedral-form.md) · [Index](../README.md) · [MLIR: Incremental Application to Graph Algorithms in ML Frameworks →](06-mlir-for-graph-algorithms.md)
