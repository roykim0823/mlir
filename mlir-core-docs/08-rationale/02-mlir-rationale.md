...
> **Section:** Rationale and Design History · document 2 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Rationale/Rationale/](https://mlir.llvm.org/docs/Rationale/Rationale/) · source [`mlir/docs/Rationale/Rationale.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Rationale/Rationale.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The original rationale for MLIR's core design — why operations are the single extensibility
mechanism, why regions exist, why the type system is what it is, why SSA with block arguments rather
than phi nodes, and a long series of specific decisions with the reasoning attached.

Read it after you have used MLIR enough to have opinions. It is where you find out that most of the
things that seemed arbitrary were considered carefully, and it will change how you design your own
dialects. Some sections describe decisions later revised, so treat specifics with mild suspicion and
the reasoning as durable.

**Read first**

- [MLIR Language Reference](../01-core-ir/01-language-reference.md)
- Enough hands-on experience to have found something surprising.

**What you should be able to do after this page**

- Explain the reasoning behind MLIR's core structural decisions.
- Apply the same reasoning when designing your own abstractions.
...
[specification document](../01-core-ir/01-language-reference.md) and other content is hosted elsewhere.
...
[affine.apply operations](https://mlir.llvm.org/docs/Dialects/Affine/#affineapply-affineapplyop) or
through the map attributes of
[affine operations](https://mlir.llvm.org/docs/Dialects/Affine/#operations)) to precisely analyze
references at compile-time using polyhedral techniques. This is possible because
of the
[restrictions on dimensions and symbols](https://mlir.llvm.org/docs/Dialects/Affine/#restrictions-on-dimensions-and-symbols).
...
MLIR Regions represent SSA using "[block arguments](../01-core-ir/01-language-reference.md#blocks)"
...
bit width. MLIR makes such [data layout information](../05-data-representation/01-data-layout-modeling.md)
...
[type aliases](../01-core-ir/01-language-reference.md#type-aliases).

### Tuple types

The MLIR type system provides first class support for defining
[tuple types](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Dialects/Builtin#tupletype). This is due to the fact that
...
As per the [MLIR spec](../01-core-ir/01-language-reference.md), the restrictions on dimensions and symbol
...
this document:
[ MLIR: The case for a simplified polyhedral form](04-simplified-polyhedral-form.md).
...
### The arguments worth extracting

**Block arguments instead of phi nodes.** Phi nodes are a special case every CFG transformation must
handle. Block arguments make the same information ordinary — a block is a function of its
arguments — and the special case disappears. Anyone who has written LLVM passes recognises this as a
real improvement.

**One extensibility mechanism.** Operations. Not operations plus intrinsics plus metadata plus
target-specific nodes. The uniformity is what allows generic infrastructure to exist, and it is the
decision the rest of the framework hangs from.

**Regions rather than a flat CFG.** Preserving structure is cheaper than recovering it. A loop that
stays a loop until you decide otherwise avoids the whole business of loop recognition. The cost is
that every transformation must handle nesting, which is a real cost the document does not minimise.

**Verification as a first-class concern.** Invariants that are checked are invariants that hold. The
verifier is not a debugging aid, it is part of the contract, which is why so much of ODS is about
declaring constraints.

**Progressive lowering.** Rather than one big translation, many small ones with legal IR at every
step. This is what makes the pipeline debuggable — you can print and verify at any point — and what
allows abstractions to be mixed.

**Signless integer types.** Signedness belongs to the operation, not the type. This eliminates the
sign-conversion noise that pervades IRs where `i32` and `u32` are distinct, at the cost of requiring
separate signed and unsigned operations.

### Dated in places

Some material predates the current state: the standard dialect has since been split up, properties
were introduced after this was written, and parts of the type-system discussion have been overtaken.
Cross-check specifics against the
[Language Reference](../01-core-ir/01-language-reference.md); the *reasoning* is what you are here
for.


---

[← Side Effects and Speculation](01-side-effects-and-speculation.md) · [Index](../README.md) · [Generic DAG Rewriter Infrastructure Rationale →](03-generic-dag-rewriter-rationale.md)
