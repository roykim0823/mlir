# The Case for a Simplified Polyhedral Form

> **Section:** Rationale and Design History · document 4 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Rationale/RationaleSimplifiedPolyhedralForm/](https://mlir.llvm.org/docs/Rationale/RationaleSimplifiedPolyhedralForm/) · source [`mlir/docs/Rationale/RationaleSimplifiedPolyhedralForm.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Rationale/RationaleSimplifiedPolyhedralForm.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The argument for MLIR's approach to the polyhedral model: rather than extracting loop nests into a
separate mathematical representation, optimizing there, and generating code back — the classical
approach taken by Polly, PLUTO, isl and others — MLIR keeps the loops in the IR and attaches affine
constraints to them.

Read it if you work on loop optimization, or as a well-argued case study in the general question of
whether to use a specialised representation or to enrich the main one.
...
- Explain the trade-off between an embedded and a separate polyhedral representation.
- Understand why MLIR's affine constructs look like ordinary loops rather than schedule trees.
...
### Background: what "affine" means here

The `affine` dialect (documented at <https://mlir.llvm.org/docs/Dialects/Affine/>, outside this
collection's scope) restricts loop bounds and memory subscripts to affine functions of enclosing loop
induction variables and symbols. That restriction makes dependence analysis decidable, and therefore
makes loop interchange, tiling, fusion and skewing provable rather than heuristic.

The vocabulary it introduces — `AffineMap`, `IntegerSet`, and the distinction between *dimensions*
(which vary across the iteration space) and *symbols* (which are loop-invariant) — is used well
beyond that dialect: buffer layouts and structured-operation indexing maps are all affine maps. So
this document is worth reading even if you never write an affine loop.

### The classical approach and its costs

Extract to a polyhedral representation, optimize with powerful mathematical machinery, generate code
back. The optimization is genuinely strong. The costs are structural:

- **Extraction is partial.** Only affine regions qualify, so the boundary must be managed.
- **Code generation is lossy.** The regenerated loop nest may not resemble the original, which makes
  debugging and diagnostics difficult.
- **Round-tripping is expensive**, so it happens once, in a phase, rather than being interleaved with
  everything else.
- **Two representations must be kept consistent**, which is a persistent source of bugs.

### MLIR's position

Keep the loops; attach affine maps and integer sets. You get dependence analysis and the standard
transformations, applied in place, interleaved with everything else, with the IR remaining
inspectable throughout. You give up some of the more global scheduling power a full polyhedral
scheduler provides.

### How it looks in retrospect

Reasonable, with the caveat that machine-learning code generation has largely moved to the structured
operations path rather than to affine loop transformations. The polyhedral machinery remains valuable
for genuinely affine HPC-style code, and the affine map vocabulary is used throughout MLIR
regardless.

The general lesson — that a representation you can keep, inspect and interleave often beats a more
powerful one you must round-trip through — is the transferable part, and it recurs in the
bufferization design too.


---

[← Generic DAG Rewriter Infrastructure Rationale](03-generic-dag-rewriter-rationale.md) · [Index](../README.md) · [Linalg Dialect Rationale: The Case For Compiler-Friendly Custom Operations →](05-structured-ops-rationale.md)
