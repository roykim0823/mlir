...
> **Section:** Rationale and Design History · document 3 of 7  
> **Upstream:** [https://mlir.llvm.org/docs/Rationale/RationaleGenericDAGRewriter/](https://mlir.llvm.org/docs/Rationale/RationaleGenericDAGRewriter/) · source [`mlir/docs/Rationale/RationaleGenericDAGRewriter.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Rationale/RationaleGenericDAGRewriter.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The design argument behind the pattern rewriting infrastructure, including a comparison with LLVM's
InstCombine and DAGCombine, with instruction selection systems, and with term-rewriting literature.

Worth reading if you are designing your own pattern set at scale, or if you want to understand the
trade-offs behind the greedy driver's behaviour — the convergence issues, the benefit model, and the
choice not to build a general term rewriting engine.
...
- Explain the trade-offs the rewriter's design makes.
- Understand why declarative pattern description was pursued.
...
please look at the main [Pattern Rewriting document](../03-passes-and-rewriting/03-pattern-rewriting.md).
...
[`fold`](../03-passes-and-rewriting/02-operation-canonicalization.md#canonicalizing-with-the-fold-method) routine, which
...
### The problems it identifies in prior systems

**InstCombine** is thousands of lines of hand-written C++ with implicit ordering dependencies, no
declarative description, and the well-known property that adding a pattern can break an unrelated
one. It works, and its maintenance cost is high.

**DAGCombine and instruction selection** are more declarative but tied to a fixed type system and a
fixed set of nodes — not viable for an open, extensible IR.

MLIR's response: a generic driver plus declarative pattern description (DRR, then PDL and PDLL), with
benefit-based selection rather than implicit ordering. The document is candid that this does not
solve everything — the convergence problem in particular is inherent to fixpoint rewriting, not an
implementation defect.

### The design tension it surfaces

Pattern-based rewriting is composable and locally reasoned about, which is why it scales to many
contributors. It is also globally unpredictable: the interaction of two hundred patterns is not
something anyone reasons about directly, and the outcome depends on application order in ways that
are hard to bound.

That tension is not resolved, in MLIR or anywhere else, and recognising it is useful. It explains why
phase ordering keeps being a problem, and why explicit scheduling of transformations is attractive
for the parts of a pipeline where you need determinism.

### What to take from it

If you are building a large pattern set, two practical consequences follow from this document's
analysis: keep patterns strictly directional so the fixpoint terminates, and separate pattern sets
that must run in a particular order into separate driver invocations rather than relying on benefit
values to encode a phase ordering.


---

[← MLIR Rationale](02-mlir-rationale.md) · [Index](../README.md) · [The Case for a Simplified Polyhedral Form →](04-simplified-polyhedral-form.md)
