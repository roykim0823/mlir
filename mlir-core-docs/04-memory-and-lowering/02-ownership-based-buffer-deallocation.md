...
> **Section:** Memory and Lowering · document 2 of 3  
> **Upstream:** [https://mlir.llvm.org/docs/OwnershipBasedBufferDeallocation/](https://mlir.llvm.org/docs/OwnershipBasedBufferDeallocation/) · source [`mlir/docs/OwnershipBasedBufferDeallocation.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/OwnershipBasedBufferDeallocation.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Bufferization allocates. Something must free. This pass does it by tracking *ownership* — a
runtime-materialized notion of which buffer a given point in the program is responsible for
releasing — rather than by attempting a purely static liveness analysis, because static analysis
cannot resolve cases where control flow decides which of two buffers a value refers to.

Read it after bufferization. The ownership indicator concept is the part worth understanding: it
explains why the pass sometimes emits conditional deallocations and boolean ownership variables that
look, at first glance, like a needlessly complicated way to free memory.
...
- Explain why ownership is tracked at run time rather than inferred statically.
- Read the generated ownership indicators and conditional deallocations.
- State the assumptions the pass makes about its input.
...
### The case that forces the design

```mlir
%buf = scf.if %cond -> memref<?xf32> {
  scf.yield %allocated : memref<?xf32>     // we own this
} else {
  scf.yield %argument  : memref<?xf32>     // the caller owns this
}
```

At the join point, whether `%buf` must be freed depends on which branch executed. No static analysis
can answer that in general, so the pass materialises the answer: an `i1` ownership value flows
alongside the buffer, and the deallocation becomes conditional on it.

Once you have seen this example, the pass's output stops looking strange.

### What it assumes about its input

The pass has preconditions, notably about how buffers are yielded from regions and about operations
whose aliasing behaviour it cannot see. Running it on IR that does not satisfy them produces leaks or
double frees rather than an error, which is the most dangerous failure mode in this section. Run it
on the output of One-Shot Bufferize, in the position the documented pipeline puts it, before assuming
a bug is in the pass.

### After it runs

Two cleanup passes matter:

- **buffer deallocation simplification** removes ownership indicators that turn out to be constant,
  which removes most of the conditional deallocations in practice;
- **buffer hoisting and loop hoisting** move allocations out of loops so a buffer is allocated once
  rather than per iteration. On a tiled loop nest this is frequently the difference between a
  pipeline that is competitive and one that is not.

### Verifying it worked

`mlir-opt` cannot tell you that you have no leaks. Run the generated code under a sanitizer or leak
checker on real inputs. For accelerator targets where that tooling does not exist, a custom-allocator
shim that counts allocations and frees is the usual substitute, and it is worth building early —
retrofitting leak detection after a pipeline is in production is much harder than adding a counting
allocator at the start.

### Relationship to ordinary reference counting

The ownership indicator is not a reference count. It is a single-owner protocol: exactly one point in
the program is responsible for freeing a given allocation, and the indicator records whether *this*
point is that one. That is cheaper than reference counting and sufficient because the compiler
controls all the aliasing it introduced.


---

[← Bufferization](01-bufferization.md) · [Index](../README.md) · [LLVM IR Target →](03-llvm-ir-target.md)
