...
> **Section:** Memory and Lowering · document 3 of 3  
> **Upstream:** [https://mlir.llvm.org/docs/TargetLLVMIR/](https://mlir.llvm.org/docs/TargetLLVMIR/) · source [`mlir/docs/TargetLLVMIR.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/TargetLLVMIR.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

The main exit route for CPU targets, and the most detailed account of a real lowering in the
documentation set. Two distinct stages: *conversion* from higher dialects to the `llvm` dialect,
which is pattern-based and lives inside the pass infrastructure, then *translation* from the `llvm`
dialect to actual LLVM IR, which is mechanical and lives outside it.

The section on memref descriptors and calling conventions is the one to read carefully. It explains
what a buffer reference actually becomes at the ABI level, which is the information you need whenever
MLIR-generated code has to interoperate with hand-written C.

**Read first**

- [Dialect Conversion](../03-passes-and-rewriting/07-dialect-conversion.md)
- [Bufferization](01-bufferization.md)

**What you should be able to do after this page**

- Assemble a conversion pipeline that terminates in the LLVM dialect.
- Predict the C signature of a function taking or returning memrefs.
- Choose between the descriptor, bare-pointer and C-wrapper conventions.
- Extend the type converter and translation for your own types.
...
1.  **conversion** of the IR to a set of dialects translatable to LLVM IR, for
    example [LLVM Dialect](https://mlir.llvm.org/docs/Dialects/LLVM/) or one of the hardware-specific
    dialects derived from LLVM IR intrinsics such as [X86](https://mlir.llvm.org/docs/Dialects/X86/)
    or [ArmNeon](https://mlir.llvm.org/docs/Dialects/ArmNeon/);
...
[dedicated document](https://mlir.llvm.org/docs/SPIRVToLLVMDialectConversion/).
...
The process relies on the [Dialect Conversion](../03-passes-and-rewriting/07-dialect-conversion.md)
infrastructure and, in particular, on the
[materialization](../03-passes-and-rewriting/07-dialect-conversion.md#type-conversion) hooks of `TypeConverter`
...
The types [compatible](https://mlir.llvm.org/docs/Dialects/LLVM/#built-in-type-compatibility) with the
...
specified by the [data layout](../05-data-representation/01-data-layout-modeling.md) of the closest module. For
...
**[strided form](https://mlir.llvm.org/docs/Dialects/Builtin/#strided-memref)** can be converted to the
...
be [bufferized](01-bufferization.md) before being converted.
...
### Conversion, then translation

```
your dialects
  → convert-*-to-llvm passes            (pattern-based, MLIR passes)
    → llvm dialect
      → mlir-translate --mlir-to-llvmir (mechanical export)
        → LLVM IR → your usual backend
```

A failure in the first stage is a legalization problem
([Dialect Conversion](../03-passes-and-rewriting/07-dialect-conversion.md)) and is debugged with
`--debug-only=dialect-conversion`. A failure in the second is almost always an unhandled operation or
type in the translation, and produces a different class of error message. Knowing which stage failed
is most of the diagnosis.

### Background: what the `llvm` dialect is

LLVM IR modelled as an MLIR dialect — LLVM types, operations, attributes and metadata as MLIR
constructs — so that the final stage of a pipeline stays inside MLIR and only leaves at the very end.
You can run MLIR passes on it, mix it with other dialects during a partial conversion, and keep MLIR
locations for diagnostics.

Two differences from LLVM IR worth knowing: there is no `phi`, because MLIR block arguments do that
job; and intrinsics appear as operations (`llvm.intr.*`) rather than as calls to declared functions.
Pointers are opaque (`!llvm.ptr`), matching modern LLVM — examples written before that transition
show typed pointers and will not parse.

### The memref ABI — the part that surprises people

A `memref<?x?xf32>` becomes a descriptor struct:

```
{ float* allocated, float* aligned, i64 offset, i64 sizes[2], i64 strides[2] }
```

which, under the default convention, is *passed as individual scalar arguments* rather than as a
struct. So a function that looks like it takes one argument takes seven. This is why calling
MLIR-generated code from C by guessing the signature does not work.

The three conventions:

| Convention | Signature | Use when |
|---|---|---|
| Default (unpacked descriptor) | fields as separate arguments | MLIR-to-MLIR calls |
| **Bare pointer** (`use-bare-ptr-memref-call-conv`) | a single pointer | shapes are static and you control both sides |
| **C wrappers** (`emit-c-wrappers`) | an emitted wrapper taking a pointer to a descriptor struct | calling from hand-written C |

For a hand-written C interface, C wrappers plus a struct definition matching the descriptor is the
maintainable choice; bare pointers are simplest when every shape is static.

The `allocated` and `aligned` pointers being separate is not redundancy: `allocated` is what must be
freed, `aligned` is what you index from. They differ whenever the allocator over-allocates to satisfy
an alignment requirement, and confusing them produces a heap corruption that is very unpleasant to
debug.

### Index type width

`index` lowers to an integer whose width is a conversion option (`index-bitwidth`). Set it
inconsistently across passes in one pipeline and you get type mismatches at the seams between them.
Set it once and pass it everywhere.

### Extending it

For a custom type, add a conversion to the `LLVMTypeConverter`. For a custom operation, either a
conversion pattern producing `llvm` dialect operations, or — if it maps to an LLVM intrinsic —
participation in translation via `LLVMTranslationDialectInterface`. The second route is how target
dialects emit their intrinsics, and it is usually less work than it looks.

### Debugging translation failures

`mlir-translate --mlir-to-llvmir` on the isolated `llvm`-dialect module is the fastest check, and it
will name the operation it cannot translate. Verify the module first: invalid `llvm` dialect IR
frequently produces a translation crash rather than a diagnostic, so a clean run of `mlir-opt` with
verification enabled is worth doing before blaming the translator.

### Running the result

MLIR's `ExecutionEngine` JITs the LLVM IR and lets you call into it, which is how `mlir-cpu-runner`
works and how the Python bindings execute code. For testing a lowering, JITting and comparing against
a reference implementation is far more convincing than inspecting the generated IR.


---

[← Ownership-based Buffer Deallocation](02-ownership-based-buffer-deallocation.md) · [Index](../README.md) · [Data Layout Modeling →](../05-data-representation/01-data-layout-modeling.md)
