...
> **Section:** Memory and Lowering · document 1 of 3  
> **Upstream:** [https://mlir.llvm.org/docs/Bufferization/](https://mlir.llvm.org/docs/Bufferization/) · source [`mlir/docs/Bufferization.md`](https://github.com/llvm/llvm-project/blob/main/mlir/docs/Bufferization.md)  
> **License:** upstream text is Apache-2.0 WITH LLVM-exception.

## Orientation

Above the line, values have `tensor` type: no address, no aliasing, freely duplicable. Below it,
values have `memref` type: real storage, with allocation, aliasing and lifetime. Bufferization is
the phase that crosses the line, and doing it well is the difference between a pipeline that
allocates a fresh buffer for every intermediate and one that updates in place.

This page documents One-Shot Bufferize, the current framework: how it analyses tensor uses to decide
what can be written in place, how `BufferizableOpInterface` lets your operations participate, and
what the layout and ownership rules are.

**Read first**

- [MLIR Language Reference](../01-core-ir/01-language-reference.md)
...
- Explain how One-Shot Bufferize decides between in-place update and a copy.
- Make your own operations bufferizable.
- Read and act on a bufferization analysis result.
...
[insert buffer deallocation ops](02-ownership-based-buffer-deallocation.md) so that
...
[Ownership-based Buffer Deallocation passes](02-ownership-based-buffer-deallocation.md).
...
### Background: the two types this phase moves between

The dialect reference is outside this collection's scope, so briefly:

`tensor<4x8xf32>` is a **pure value**. It has no address. Two tensors with the same contents are
indistinguishable, so the compiler may duplicate, reorder, sink or eliminate them at will. This is
why optimization above bufferization is cheap: there is no aliasing question to answer.

`memref<4x8xf32, strided<[8,1], offset: 0>, 3>` is a **reference to storage**. It carries a shape, an
optional *layout* mapping logical indices to linear offsets, and an optional *memory space* tag
naming which memory it lives in — the mechanism you use to distinguish scratchpad from main memory
on an accelerator. Two memrefs can alias the same allocation, which is exactly the complexity that
tensors do not have.

Bufferization assigns storage to values and rewrites operations to work on that storage.

### Why this is hard

A tensor operation is `out = f(in)`. A buffer operation is `write into out_buf, reading in_buf`. The
rewrite is only valid in place if nothing else needs `in_buf`'s old contents afterwards. So
bufferization is fundamentally an aliasing and liveness problem, and a conservative answer costs a
copy — potentially of a very large tensor.

One-Shot Bufferize gets its name from doing the analysis *once*, over the whole function, before
rewriting — as opposed to the older incremental approach that bufferized dialect by dialect and
inserted copies at every boundary, which then had to be cleaned up afterwards with limited
information.

### Destination-passing style is the enabling convention

Operations in destination-passing style take an output tensor operand representing the
already-allocated destination and produce a tensor result that is conceptually the updated
destination. This is what makes in-place bufferization *expressible*: the operation already names the
buffer it wants to write into, so the analysis only has to prove the old contents are dead.

`DestinationStyleOpInterface` is how an operation declares this. If you are designing a dialect whose
operations will be bufferized, adopting destination-passing style from the start is one of the
highest-leverage decisions available — retrofitting it later means changing every operation's
signature and every pattern that builds them.

### Reading the analysis

`--one-shot-bufferize="test-analysis-only"` annotates the IR with the decisions rather than
performing them, marking which operands bufferize in place and which force a copy. When you find an
unexpected copy in the output, this is the tool: it shows you the conflicting use directly instead of
leaving you to infer it.

Common causes of an unwanted copy:

- a tensor used again after the operation that would have written in place;
- a value crossing a function boundary where the analysis cannot see all callers;
- an operation that does not implement `BufferizableOpInterface` and so is treated conservatively;
- returning a tensor that aliases a function argument.

### Function boundaries

Bufferizing across calls requires knowing the callee's aliasing behaviour. There are options for
whether to bufferize function boundaries at all and how to treat unknown functions. Module-level
bufferization handles the whole call graph; the alternative is to bufferize each function in
isolation with a conservative boundary convention, which costs copies at every call.

### Ordering in a pipeline

Bufferize **late**. Every optimization is easier on tensors: no aliasing, no lifetime, free
duplication and reordering. Fusion, tiling and most rewriting should have happened already. After
bufferization you are in the world of copy elimination and buffer reuse, which is a harder game
played with less information.

The standard tail is: bufferize →
[buffer deallocation](02-ownership-based-buffer-deallocation.md) → buffer optimization passes →
[lowering to LLVM](03-llvm-ir-target.md).

### Making your operations participate

Implement `BufferizableOpInterface`, which asks your operation:

- which operands may bufferize in place (`bufferizesToMemoryRead`, `bufferizesToMemoryWrite`);
- which result aliases which operand (`getAliasingOpOperands`);
- how to perform the rewrite (`bufferize`).

Implementing it as an external model ([Interfaces](../01-core-ir/07-interfaces.md)) keeps your
dialect free of a dependency on the bufferization framework, which is how upstream does it.


---

[← Writing Dataflow Analyses](../03-passes-and-rewriting/08-writing-dataflow-analyses.md) · [Index](../README.md) · [Ownership-based Buffer Deallocation →](02-ownership-based-buffer-deallocation.md)
